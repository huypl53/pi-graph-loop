...
- **swarm (H5 live-found, herdr idle workspace root tab)**: `workspace create` always spawns the agents workspace with an idle shell root tab (label '1') that the swarm never uses — it lingered forever beside the agent tabs (user-visible in the herdr UI). herdr closes a workspace when its LAST tab closes, but the ws survives losing the root tab while an agent tab exists (live-verified). Fix: the driver closes the workspace root tab one-shot after the first agent tab is created (best-effort, logged on failure). Red-green vs real binary: `.pi/swarm-uat/runs/herdr-ws-root-tab-red/` (2 tabs: idle '1' + agent) vs `-green/` (1 tab: agent only). Lane: `scripts/uat/herdr-ws-root-tab.mjs`.
