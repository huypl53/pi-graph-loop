...
8. **Handle root routing explicitly.**
   - Current swarm does not register `root` as a recipient agent.
   - Review agents could not `swarm_send_message` to `root` because the extension rejects unknown recipients.
   - V1 task flow should either create a pseudo-agent mailbox for `root` or use a real coordinator agent id as `replyTarget`.
...
Both reviewers discovered a real swarm limitation: child agents cannot currently send a result message to `root` unless `root` exists in `swarm-state.json.agents`.

Observed behavior from `task-design-implementability-223543`:

```text
swarm_send_message failed with Unknown swarm agent: root
```

This is valuable evidence for the task graph design: task assignment must include an explicit `replyTarget`, or the extension must create a routable root pseudo-agent.

**Resolved.** The extension now lazily creates/refreshes an `root` pseudo-agent (also on the root's own `session_start`). Delivery to it is mailbox-only and surfaced by an auto-pump (`pumpRootMailbox`) plus `swarm_check_mailbox` / `swarm_agent_status`; it is not treated as a tmux-injection failure. See [`docs/swarm.md`](./swarm.md) ("Task graph and closure") and [`docs/swarm-task-graph.md`](./swarm-task-graph.md) ("Root-directed replies").
