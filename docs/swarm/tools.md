...
- `/swarm pool rotate now` — orchestrator-only; force-swap the current slot to a healthy alternative via `pi.setModel()`. Bypasses the Issue 17 engine-retry gate and the gate's streak count; traces `pool.swap_forced_by_manual_override`. Bumps the swap-chain counter (`MAX_SWAP_CHAIN=2`). Refuses (with a warning notify) if every alternative is benched or the picked slot has no resolvable model registry entry. No new public tool — slash command only.
