...
| 22 | Model pool per-slot roles filter | modelPool[i].roles?: string[]. When set, slot only eligible for pickSlot if agent's roleKind is in the list. Manual /swarm pool rotate now bypasses filter. Empty/absent = available for all roles. pickSlot gets new optional roleKind parameter; swarm_spawn_agent passes spawned agent's roleKind. | Queued: `model-pool-roles-filter` |
