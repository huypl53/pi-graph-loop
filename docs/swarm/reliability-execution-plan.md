...
| 18 | Swarm goal: idle-streak nudge with back-off | swarm_set_goal + swarm_mark_goal_done tools + /swarm goal set/done commands. Orchestrator pump nudges when goal set + all agents idle. Anti-loop: 3 nudges without resolve → skip 2 ticks. | Queued: `swarm-goal-idle-nudge` |
