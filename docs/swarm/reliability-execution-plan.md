...
| 22 | Model pool per-slot roles filter | `modelPool[i].roles?: string[]`. Optional slot-level allow-list of roleKind names. New `slotMatchesRole` helper + `pickSlot` optional `roleKind` / `bypassRolesFilter` params. `parseModelPool` (`session.ts`) forwards `roles` into the parsed `ModelSlot` so the filter is observable end-to-end. `swarm_spawn_agent` threads the spawned agent's roleKind into `pickSlot`; if every slot is filtered out, log `pool.role_filter_all_filtered_fallback` trace + retry without filter (so the worker always starts). In-process auto-swap (`hooks.ts:turn_end`) reads `agent.roleKind` from `state.agents[agentId].roleKind` under lock; the `pool.swap` trace carries `roleKind` + `rolesFilterMatched`. `/swarm pool list` renders a `roles=` column when ANY slot has roles set; `/swarm pool show` adds a `roles=[…]` line per slot. Manual `/swarm pool rotate now` passes `bypassRolesFilter:true`; trace carries `rolesIgnored:true` + `agentRoleKind`. | Implemented: `model-pool-roles-filter` (pending test/review) |
| 23 | Task-liveness idle-nudge (engine gap) | Observed live: task in_progress, reviewer settled, all agents idle incl. orchestrator, no goal set → no nudge fired (Issue 18 only nudges on goal). Fix: when any task is non-terminal with NO node assigned/in_progress and NO agent busy for >N sec, emit orchestrator nudge (reuse goal-nudge back-off/cap machinery) carrying next ready node. Trace `task.idle_stall_nudge`. No auto-assign. | Queued (roadmap addition from live incident 2026-08-28) |
| 24 | Assignment ownership self-healing (engine gap) | Live incident: spawn + direct assignment message without swarm_assign_task left node assignee=null; agent finishing work was refused by NODE_ASSIGNEE_MISMATCH despite zero contention. Fixes: (a) allow non-orchestrator agent to CLAIM a node whose assignee is null (no owner = no dispute); (b) auto-set node assignee when an assignment message carrying taskId/nodeId is delivered to an agent; (c) update Issue 14 warning to also hint assign_task; (d) reject errors on task-graph tools carry actionable remediation hints (e.g. NODE_ASSIGNEE_MISMATCH on unassigned node must say how to resolve, not just refuse); (e) warn-at-detection: when any agent invokes a task-graph tool on a task where its own assignment message is delivered but node assignee is null, emit immediate warning trace instead of only failing at update time; (f) extend Issue 14 detector: when a message carrying taskId/nodeId is delivered to an agent that is NOT the node assignee, emit warning trace immediately (spawn-with-message-but-no-assign covers this incident: message satisfied Issue 14 yet node stayed orphaned). | Queued (roadmap addition from live incident 2026-08-28) |
