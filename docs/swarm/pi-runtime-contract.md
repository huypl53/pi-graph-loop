...
| **P-8** | CT-8 | Pure unit test: read orchestrator agent record; assert `tmuxTarget === "unknown"` | LOW | **RE-SCOPED 2026-09-02 (source-import unit)** — CT-8 does not require a real lane; it exercises `ensureOrchestrator` from `extensions/swarm/src/identity.ts:80,106` directly (the source-import unit) and asserts the returned record has `tmuxTarget === "unknown"`, `id === "orchestrator"`, `roleKind === "orchestrator"`. No provider transcript required. See `extensions/swarm/ct-phase2-probes.test.mjs` §CT-8 + the harness transcripts (kept on disk as debug artifacts only, NOT cited as §11 evidence). F2 framing **CONFIRMED true**: orchestrator pseudo-agent intentionally has `tmuxTarget === "unknown"` (mailbox-only delivery). |
