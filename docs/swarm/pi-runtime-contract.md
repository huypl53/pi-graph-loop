...
| **P-3** | CT-3 | Mock-llm fixture: idle agent + `sendMessage({nextTurn, triggerTurn: true})`; assert no turn fires; assert message only on next user prompt | LOW | **EXECUTED 2026-09-02 PASS** — verdict: `CONTRACT_CONFIRMED` (3/3 sub-cases); evidence: `.pi/mock-llm/transcripts/ct3-nextturn-idle/2026-09-02T11-39-48-870Z-mockllm-68acb25e-1bde-450e-ace2-d672ea68adeb.json` (CT-3.A: sendMessageCallCount=1, returnIsUndefined=true, same-tick deltaMs<5, opts.deliverAs=`nextTurn`, opts.triggerTurn=true), `…2613ba18-fc42-4211-8115-08d66b582157.json` (CT-3.B: isIdleAfterNextTurnSend=true, turnCursorAdvancedSinceInjection=false, nextTurnTriggerTurnWasIgnored=true), `…5c9a9080-8482-40ab-a8d1-3668045ce90e.json` (CT-3.C: surfacesOnNextUserPrompt=true, contextIncludesQueuedCustom=true); per `extensions/swarm/ct-phase2-probes.test.mjs` §CT-3 (`node extensions/swarm/ct-phase2-probes.test.mjs` → `CT-PHASE2-PROBES PASS (38 passed, 0 failed)`). Fixture (documentary): `extensions/mock-llm/fixtures/ct3-nextturn-idle.jsonl`. F8 footgun **CONFIRMED safe**: `nextTurn` + `triggerTurn:true` does NOT start a turn while idle; queued message surfaces only on next user prompt. |
| **P-4** | CT-4 | Mock-llm fixture: trigger auto-compaction retry; assert `ctx.isIdle() === false` | MEDIUM | **EXECUTED 2026-09-02 PASS** — verdict: `CONTRACT_CONFIRMED` (2/2 sub-cases); evidence: `.pi/mock-llm/transcripts/ct4-compaction-retry/2026-09-02T11-39-49-624Z-mockllm-e1286c73-28a5-4f09-ba45-a9de8fbdb232.json` (CT-4.A: isIdleSampleCountDuringCompaction=4 sampled at 0/100/250/500ms, isIdleFalseCountDuringCompaction≥1, compactionRetryObservedFalse=true), `…b5ee49c6-1213-4e69-8088-46c73166322f.json` (CT-4.B: midSample.isIdle=false, postSample.isIdle=true, runtimeStatusFlippedToIdle=true); per `extensions/swarm/ct-phase2-probes.test.mjs` §CT-4. Fixture (documentary): `extensions/mock-llm/fixtures/ct4-compaction-retry.jsonl`. §4 idle/streaming/pending table behavior **CONFIRMED**: `ctx.isIdle() === false` during compaction retry window, `ctx.isIdle() === true` once compaction completes + `runtimeStatus === "idle"`. Note: this probe pins down the §4 contract under auto-compaction; no §10 F-row applies. If the §4 idle semantics change, this probe would fail and a NEW F-row would need to be filed. |
| **P-5** | CT-5 | Pure unit test: read `ctx.signal` from session_start handler; assert undefined | MEDIUM | **EXECUTED 2026-09-02 PASS** — verdict: `CONTRACT_CONFIRMED`; evidence: `.pi/mock-llm/transcripts/ct5-ctx-signal/2026-09-02T11-39-49-625Z-mockllm-22eae135-0a83-469a-a5ef-2da1086f257d.json` (ctxSignalReads=0, ctxSignalUndefinedAtSessionStart=true, capturedSignalType=`undefined`, isAbortSignalInstance=false); per `extensions/swarm/ct-phase2-probes.test.mjs` §CT-5. F5 footgun **CONFIRMED safe**: `ctx.signal` is `undefined` at the `session_start` boundary; not an `AbortSignal` instance. Background timers + module-load captures started inside `session_start` MUST thread `ctx.signal` from a subsequent `tool_result` or `message_update` handler — they cannot rely on the boundary-time signal. |
| **P-6** | CT-6 | Pure unit test: capture ctx, await `ctx.reload()`, then use the captured ctx; assert throws `"This extension ctx is stale..."` | MEDIUM | **EXECUTED 2026-09-02 PASS** — verdict: `CONTRACT_CONFIRMED`; evidence: `.pi/mock-llm/transcripts/ct6-stale-ctx/2026-09-02T11-39-49-625Z-mockllm-4d521018-c8bc-499e-8810-5ae6b526dcae.json` (staleCtxUseCallCount=1, staleCtxThrowObserved=true, throwMessageMatchesStalePattern=true matching `/This extension ctx is stale/`, capturedThrowMessage=`"This extension ctx is stale; it was torn down by ctx.reload(). Use the ctx returned from withSession() instead."`); per `extensions/swarm/ct-phase2-probes.test.mjs` §CT-6. F6 footgun **CONFIRMED safe** on the throw-on-stale-use side: captured `ctx.registerTool(...)` (and any other captured `pi`/`ctx` method) throws the `This extension ctx is stale...` error after `await ctx.reload()`. The pre-reload call succeeds (CT-6 baseline: `preReloadRegisterResult.id === "ok"`). Combined with CT-1.C (which covers the still-callable-but-error-event side), the F6 row is fully pinned: pre-reload captured handles throw on use after `reload` / `newSession` / `switchSession` / `fork`. |
| **P-7** | CT-7 | Mock-llm fixture: queue a follow-up + emit `agent_end`; assert `agent_settled` is later | LOW | **EXECUTED 2026-09-02 PASS** — verdict: `CONTRACT_CONFIRMED` (2/2 sub-cases); evidence: `.pi/mock-llm/transcripts/ct7-end-vs-settled/2026-09-02T11-39-49-625Z-mockllm-f948339d-2a0d-425e-a8c8-d8dbe48d187c.json` (CT-7.A: agentEndEmittedCount≥1, agentSettledEmittedCount=0, agentSettledNotYetAtMidStream=true), `…960962c8-1a54-4556-85dd-9d52bc24e3d6.json` (CT-7.B: agentEndEmittedCount≥1, agentSettledEmittedCount≥1, agentSettledAfterFollowUp=true with ordering `agent_end < followUp_consumed < agent_settled`, emissionOrderingMatches=true); per `extensions/swarm/ct-phase2-probes.test.mjs` §CT-7. Fixture (documentary): `extensions/mock-llm/fixtures/ct7-end-vs-settled.jsonl`. F7 footgun **CONFIRMED safe**: `agent_end` is NOT the "fully idle" boundary; `agent_settled` fires AFTER the queued follow-up is consumed. Swarm code that needs "no automatic continuation will run" MUST listen for `agent_settled`, not `agent_end`. |
| **P-8** | CT-8 | Pure unit test: read orchestrator agent record; assert `tmuxTarget === "unknown"` | LOW | **EXECUTED 2026-09-02 PASS** — verdict: `CONTRACT_CONFIRMED`; evidence: `.pi/mock-llm/transcripts/ct8-orchestrator-unknown/2026-09-02T11-39-49-635Z-mockllm-9c2f1f24-a416-4919-94f6-5bd8016801fc.json` (orchestratorAgentRecordFound=true, orchestratorTmuxTargetMatchesUnknown=true with literal string `"unknown"`, orchestratorIdMatches=true with `id === "orchestrator"`, orchestratorRoleKindMatches=true with `roleKind === "orchestrator"`); record dump: `{id: "orchestrator", roleKind: "orchestrator", tmuxTarget: "unknown", tmuxWindow: "orchestrator", status: "running", runtimeStatus: "idle"}`; per `extensions/swarm/ct-phase2-probes.test.mjs` §CT-8 via `extensions/swarm/src/identity.ts:80,106` (`ensureOrchestrator`). F2 framing **CONFIRMED true**: orchestrator pseudo-agent is intentionally created with `tmuxTarget === "unknown"` because it is a human-driven coordinating session with NO dedicated swarm tmux pane; peer messages are mailbox-only delivery. |
