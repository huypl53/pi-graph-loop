...
| **F18** | "The goal idle floor consults task-graph state (emission `active_task` scan + R19 `deferred_actionable_graph`; surface `liveGraphActionable` leg)" — i.e. surface-time revalidation must agree with emission-time gating ALSO for task-state independence | `extensions/swarm/src/nudges/goal-epoch.ts` — `evaluateIdleGoalNudgeLocked` (pre-R27) + `extensions/swarm/src/surface.ts` goal-key branch of `staleSurfaceReason` (pre-R27 `liveGraphActionable` leg) | **R27** | **HIGH** | **FIXED 2026-09-07 (R27)**: the goal floor is TASK-STATE-INDEPENDENT. Emission gates removed entirely (`findAssignedOrInProgressTaskWork` active-task suppression + `hasActionableGraphWork` R19 defer); the surface goal-key branch drops the `liveGraphActionable` leg (only `idle_epoch_advanced` remains). Debounce replaces the interval anchor: a consecutive-check streak (`goalIdleCheckCount`/`goalIdleLastCheckAt`, sampled at `PI_SWARM_GOAL_IDLE_CHECK_INTERVAL_MS` default 10s, emitting at `PI_SWARM_GOAL_IDLE_CHECKS_REQUIRED` default 3 consecutive idle samples; any busy sample resets). The nudge now fires on the exact live shape that pre-R27 silenced it forever: a live `in_progress` node assigned to an idle worker. `nudgeIntervalMs`/`PI_SWARM_GOAL_NUDGE_IDLE_INTERVAL_MS` DORMANT (F4 resolved-by-design). R10-1 boundary counters: C-R27-1 (`goal.idle_check count 1→2→3` trace), C-R27-2 (`goal.idle_nudge` emitted ON the 3rd sample while node still in_progress+assigned — proven in the tmux lane trace), C-R27-3 (real `pi.sendMessage` via `mailbox.root_pump count:1` with the nudge id), C-R27-4 (L4: nudge body present in the mock-llm transcript request preview), C-R27-5 (text-only ack → `goal.nudge.turn_no_resolve_action`, R16 semantics hold). See roadmap Row R27 + operations.md R27 + `extensions/swarm/tests/r27-goal-task-independent-idle-nudge.test.mjs` (9/9 offline real-pump lane) + `tmux-snapshots/r27-validation/`. |
| **F19** | "Unit tests that import evaluators through the `reconcile.ts` barrel constitute pump coverage" — false: at HEAD the pump's own call sites in `surface.ts` referenced `checkStallNotificationStale` and `evaluateIdleGoalNudgeLocked` WITHOUT importing either, so every real root pump tick crashed with ReferenceError (`goal.nudge.error` / staleSurfaceReason throw) while the unit suite stayed green | `extensions/swarm/src/surface.ts` lines ~399 & ~573 (pre-R27-B call sites; post-Phase-7 the pump lives in `src/surface/pump.ts` and the pinned imports remain on the `src/surface.ts` facade) | **R27-B** | **HIGH** | **FIXED 2026-09-07 (R27-B)**: the two imports were added and pinned by `extensions/swarm/tests/r27b-pump-import-guards.test.mjs` (real `pumpRootMailbox` probe asserting zero `goal.nudge.error` + `goal.idle_check` present; `staleSurfaceReason` graph-advance-key probe; static import-integrity check, 9/9). Verified pre-existing by replaying the probes against HEAD `surface.ts` in the baseline worktree — identical crashes. Contract amendment: any change touching pump-callable identifiers must ship at least one guard that executes the pump path itself or a static import-integrity check (extends R10-1 discipline). See roadmap Row R27-B. |
| **F20** | "Delivering N pending mailbox messages to root via followUp (`deliverAs: 'followUp'`) processes them efficiently in Pi runtime" — false: each followUp is converted by Pi runtime into a sequential separate LLM turn, burning context window (e.g. 467k tokens in pane %141) and forcing the root agent into 1-by-1 conversational ACKs; coalescing pending messages into a single batched `pi.sendMessage` call ensures exactly 1 LLM turn triggers for the entire batch. | `extensions/swarm/src/surface/pump.ts` L2 delivery site (pre-Phase-7: `src/surface.ts` line ~1250) | **R30** | **HIGH** | **FIXED 2026-09-24 (R30)**: `pumpRootMailbox` now coalesces `pending.length > 1` into a single `swarm-batch-message` via `formatSwarmBatchMessageContent`, calling `pi.sendMessage` exactly ONCE at the L2 boundary with `triggerTurn: true` instead of looping with `followUp`. All N message IDs are durably stamped in `consumerReceipts`/`delivered`. Boundary counting assertion in `extensions/swarm/tests/r30-root-message-batching.test.mjs` (13/13 passed). |
