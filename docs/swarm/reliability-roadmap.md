...
**Source:** Live incident 2026-09-02T15:26:06.708Z: `msg-1788362766708-64f55b39` (R23 implement-done result message from `r22-implementer` to `orchestrator`) was durably enqueued at 15:26:06.714Z (C1 + C2 `message.deliver.mailbox_only`; orchestrator has no tmux pane by design, R15/F3), then durably classified `node_terminal` by the per-tick actionability filter (`isActionableOrchestratorMessage`) at 15:26:06.913Z and again at 15:28:08.886Z — `notification.stale.suppressed site=orchestrator_pump.surface reason=node_terminal evidence=[node_terminal]` with ZERO `pi.sendMessage` at the swarm→Pi bridge. The orchestrator LLM never saw the result; the orchestrator's own `agent_settled` and watchdog pumps ran every 5s and kept suppressing. `mailbox.orchestrator_pump_stuck_escalated queued:3 oldestWaitMs:310277` at 15:31:01.991Z, then a manual `swarm_check_mailbox` at 15:31:09.993Z surfaced the envelope (`message.lifecycle_derived_shadow source=mailbox.surfaced stage=seen via=swarm_check_mailbox`) and the orchestrator ack'd done at 15:31:26.434Z. Sibling cases this session: my own R22 verdict and the R23B rework-result both surfaced only because I manually replied in-thread (same shape, different message class). RED artifact: `artifacts/r24-visible-surface-gap.red.mjs` (run BEFORE the fix) — C1 ✓ C2 ✓ `notification.stale.suppressed reason=node_terminal` ✓ C3/C4 (`pi.sendMessage`) === 0 ✓; CONTROL nudge on a terminal node also suppressed (gate intact, no false-positive exemption).

**Fix:** a single predicate change in `isActionableOrchestratorMessage` (the per-tick actionability filter that builds `windowMsgs` in `pumpOrchestratorMailbox`): a message whose fingerprint is `requiresAck && !requiresResponse && replyTo` set (the close-out shape — RESULT/REPLY/ASSIGNMENT messages produced by `swarm_send_message replyTo=...`) is exempted from `node_terminal` / `task_done` / `task_failed` / `task_cancelled` suppression so the recipient PM sees the outcome without manual polling. The exemption check fires BEFORE the task-terminal legs so nudges (canonical `task:<id>:node:<id>:nudge:*` idempotencyKey without `replyTo`) keep full gating. No new `pi.sendMessage` call site is introduced; the existing pump boundary counts at the real `pumpOrchestratorRealSend` counter (R10-1 boundary counter discipline preserved). The change crosses **L3 visible surface only** (a predicate flip on the existing surface decision inside the pump). L1/L2/C1/C2 (durable mailbox delivery) was always working — the gap was L3 specifically. L4/L5 (LLM consumption) is asserted only when the provider transcript proves it (the fixture's first turn ack `done` proves C5 in the live lane).

**Files touched:**
- `extensions/swarm/src/reconcile.ts` — `isActionableOrchestratorMessage` task-scoped branch: 4 new early-return predicates (`result_class_exempt_task_done/failed/cancelled/node_terminal`) gated on `isResultClass` computed ONCE at the top of the block
...
- `extensions/mock-llm/fixtures/orchestrator-visible-surface-gap.jsonl` + `selftest.test.mjs` registry count 70→71 — Pattern-2 seeded worker-result envelope + orchestrator ack lifecycle; second turn asserts replay dedupe (zero new `pi.sendMessage`)
...
- Suite lanes: idle-nudge **145/0** (was 136; +9 R24 section), swarm-goal **67/0**, stale-open-nudge **6/0**, R15-normal-orchestrator-result **15/15**, R13-orchestrator-unknown-target **24/24**, mock-llm selftest **71 models PASS**; repo-wide failures unchanged vs pre-fix baseline (agent-lifecycle 1 / functional 1 — stash-verified identical to R23C baseline)
- Real mock-llm lane: `artifacts/lanes/orchestrator-visible-surface-gap/tester-traces/events.jsonl` — `mailbox.orchestrator_pump reason:session_start count:1 ids:["msg-r24-result-1"]` (C-R24-1 boundary assertion at the real `pi.sendMessage`), `message.ack status=done` (C-R24-5 LLM consumption — fixture turn 1 ack closes the lifecycle), subsequent watchdog ticks `count:0, deferred:0` (C-R24-2 replay dedupe holds)

**Boundary counters:**
- `C-R24-1` real-boundary send: `pi.sendMessage` ≥1 at the pump in the live lane (the `mailbox.orchestrator_pump reason:session_start count:1` line in the trace)
...
- `C-R24-4` predicate unit: `isActionableOrchestratorMessage` returns `result_class_exempt_node_terminal` for `requiresAck && !requiresResponse && replyTo` on a terminal node (R24-1 / R24-8 unit assertions); returns `node_terminal` for the same fingerprint with `requiresResponse:true` or no `replyTo` (R24-6 / R24-7); returns `node_terminal` for nudges (R24-2 / R24-9)
- `C-R24-5` LLM consumption (C5): asserted from the mock-llm provider transcript — the orchestrator's first turn after the surface includes the surfaced msg and runs `swarm_ack_message status=done`, proving the LLM consumed the surfaced result
