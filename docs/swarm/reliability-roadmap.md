...
+ bounded reminders + root recovery authority
```
...
| Root | Chooses assignment, retry, reassign, cancellation, and final task closure. |
...
|---|---|---|---|
| New task is created but start node is never assigned | `swarm_create_task` stores the graph and returns the start node as ready. | None guaranteed for a fresh `ready` task. | Add an initial-ready-node nudge to root. |
| A later graph node becomes ready but remains unassigned | Graph advance reconciliation can create an action-required root message for a ready, unassigned node in an in-progress task. | Root | Do not auto-assign or auto-spawn by default. |
| Node is assigned but the worker/pane is not running | Assignment remains durable in mailbox; delivery is recorded as queued/failed. A later worker session may pull pending mailbox work. | Root sees assignment result; worker sees it when it starts. | Add clearer recovery/attention output; do not silently reassign. |
| Worker has not ACKed delivered work | Reconcile can mark `ack_missing` and boundedly re-inject a delivered-but-unacked message. | Worker, then root via status/reconcile. | No daemon runs reconcile forever; bounded reminders need a consistent policy. |
| Worker ACKs processing but settles without a result | `agent_settled` detects missing required response records, marks the agent `response_missing`, blocks reuse, and notifies root. | Root | Failed-first-delivery followed by later ACK currently has a tracking defect; see active repair. |
| Worker is idle/settled with an open assignment and no node update | `agent_settled` sends a cooldown-limited informational notification to root. | Root | Add one bounded, action-oriented reminder to worker before escalation. |
| A node is in progress for too long | `swarm_reconcile` reports advisory `task_node_nudge` after 30 minutes and `stale` after 24 hours. It does not auto-fail a node. | Tool caller / root | Surface this in one attention-oriented task view. |
| Agent process/pane dies with open work | State retains assignment and task source of truth; lifecycle/reconcile warnings expose it. | Root | Define explicit retry/reassign/cancel recovery semantics. |
| A declared rework edge should run | Existing graph currently does not correctly re-arm a failed terminal source/target cycle. | Root currently must force-reset. | Active repair must execute declared rework transitions automatically. |
...
- `functional.test.mjs` fails since `65835ca`: the same commit that added `requireRootAuthority` to `swarm_create_task` pins the test at `PI_SWARM_AGENT_ID=implementer-01`. Fix: run root-authority steps with an root-pinned subprocess or split the suite.
...
**Status:** open. **Priority:** P1. **Source:** R7 ritual (a1-a5 + root-verified goal-nudge investigation).
...
- [root-verified] Goal-nudge suppression predicate too narrow: assigned-to-root commit node doesn't count as active work — 3 nudges fired while root was mid-commit (18:33 event trail; seq 72/73/74 correct, interval 5s by design — cadence itself is NOT a bug). Clarify only the suppression scope.
...
→ root is not reliably surfaced a response-missing recovery action
```
...
→ agent is response_missing and root receives recovery nudge
```
...
stateDiagram-v2
    ...
    stale --> assigned: root retries or reassigns
    stale --> blocked: root confirms blocker
...
3. After a second bounded threshold, mark advisory stale / response missing and notify root.
...
### Unsafe automation: keep root authority
...
- allow a second live root to mutate graph/task state without a durable leader fence;
- let slash-command admin paths bypass the same root-only gates used by validated tools.
...
**Risk:** a non-root can potentially use `swarm_update_task(force=true)` to bypass assignee and lifecycle restrictions.

**Required policy:** `force` must only be accepted for the root (or an explicitly provisioned admin capability). The server-side current identity must be checked; client parameters are not authority.

**Phase 1 implementation:** `isRootAuthority()` (root-only) is checked server-side in `swarm_update_task` before any mutation. `force=true` from a non-root is rejected with `FORCE_FORBIDDEN`; `cancelTask=true` is rejected with `CANCEL_FORBIDDEN` (and requires `force=true` even for the root). Covered by `extensions/swarm/rbac-initial-ready.test.mjs`.

**Acceptance criteria:**

- Worker `force=true` attempt is rejected and traced. ✔
- Root force transition is accepted only where documented. ✔
...
**Proposal:** when a task's start node is ready and unassigned beyond a short creation grace period, send an idempotent action-required message to root:
...
**Phase 1 implementation:** `reconcileInitialReadyLocked` runs on the root pump. After `TASK_INITIAL_READY_GRACE_MS` (1 min) a ready+unassigned start node is nudged once with an exact `swarm_assign_task` call. Idempotent per `task:{taskId}:nudge:initial-ready`; bounded by `NOTIFY_DEFAULT_MAX_NUDGES` / `NOTIFY_DEFAULT_COOLDOWN_MS`; auto-clears when the node is assigned. It never assigns or spawns. Covered by `extensions/swarm/rbac-initial-ready.test.mjs`.

#### 5. Bounded worker completion reminder — **COMPLETE**

**Gap:** worker can finish implementation but forget status/result protocol.

**Proposal:** after confirmed receipt/processing and no task progress, send a single bounded worker reminder; escalate to root only if it remains unresolved.
...
pure durable attention derivation (`deriveNodeAttention` in `src/taskgraph.ts`), root-gated
...
single root-only cancellation surface — no new `swarm_cancel_task` tool. On cancel the
handler:
...
- `swarm_update_task` checks `isTaskOrNodeCancelled` BEFORE attempt fencing; rejects with
  `TASK_CANCELLED` or `NODE_CANCELLED`. Even an root `force=true` cannot revive a cancelled
...
- `swarm_ack_message` already rejects progress ACKs on superseded assignment records with
  `ASSIGNMENT_SUPERSEDED` (root may pass `waive=true`).
...
| Reassign | Old lease is superseded; a new attempt and worker are explicitly selected by root. |
...
root has stopped/pruned that worker and released or reassigned its node. The historical
...
claim it owns the released node; a stale queued event cannot notify the root as actionable;
...
regression sweep across `attempt-fencing`, `cancellation`, `multi-root`,
...
#### 12. Multi-root authority

**Decision:** strict-reject multiple live roots.

- A second live root pid is rejected with `ROOT_LEADER_DENIED`.
- The current leader lives in `SwarmState.rootLeader`.
- `ROOT_LEADER_STALE_MS` is the leadership blind-spot bound and is deliberately kept equal
...
- Slash-command helpers that mutate stop/release surfaces must also check root identity before
  entering their mutable branch.
- `swarm_send_keys` refuses to inject raw keystrokes into the root host pane by throwing
  `ROOT_PANE_REJECTED` when the resolved tmux target equals the root record's
  `tmuxTarget` (typically `"unknown"`); principle-based guard, fires on target equality rather than
  on `agentId` so ghost agents mis-stamped to the root's target are also rejected.
...
#### 15. Root wake-up escalation

Observed failure class: a node reaches done/ready, the graph-advance watcher emits its one-shot nudge into the
root mailbox, but the root (a human-paced PM session) does not act until the next turn — so the
...
- durable state only: escalation timing derives from recorded nudge timestamps in swarm state, never from pane
  idleness;
- wake-up surface: optionally escalate into the root PM pane via the existing tmux injection path
  (`send-keys`) — reusing existing surfaces, no new tools;
- respect leader gate: only the live `rootLeader` pane may be nudged (Issue 8);
...
| Root | Create task, inspect status/next nodes, assign, update, reconcile. |
...
9. Harden crash recovery and decide/enforce multi-root policy.
10. Final surface and architecture review.
11. Root wake-up escalation (§15).
...
1. Should only `root` be allowed to use forced graph transitions, or should named admin roles exist?
2. Should worker completion reminders be enabled by default, and what timeout/cap is acceptable?
3. Are multiple concurrent roots a supported deployment mode? **Answered (Issue 8): No.** Strict-reject is enforced: `SwarmState.rootLeader` records the live leader; a second live pid is rejected with `ROOT_LEADER_DENIED` on all root-authoritative tool and command paths (`multi-root-policy`).
...
6. Should recovery actions be model-callable tools, root-only tools, or slash commands only?

### Issue 81 — goal-clear authority guard — P0

**Status:** proposed. **Priority:** P0. **Source:** R9 a2 incident review.

`swarm_mark_goal_done` currently fences only on current goal identity and root authority. That is not enough to distinguish a standing user goal from a short-lived batch goal, so a batch-close path can still retire an intent that should remain durable.

**Proposal:** add durable goal origin metadata (`origin: user|root|system`, plus `authorId`/scope when useful) and refuse `swarm_mark_goal_done` for user-origin goals by default. A batch-scoped goal should be explicit and separately identified from a standing user goal.

**Acceptance criteria:**
- standing user goals cannot be silently cleared by batch completion;
- root tools can still close explicit batch-scoped goals;
- the clear path is auditable and semantically obvious.

### Issue 82 — agent retirement and heartbeat-based pane GC — P0

**Status:** **shipped** (commit pending root commit after review passes; landed in working tree at HEAD `90d16b1` + uncommitted). **Priority:** P0. **Source:** R9 a3 resource / ops review.
...
- `extensions/swarm/src/reconcile.ts:agentHeartbeatGCLocked` — 3 cheap gates + `lastProbeAt` ledger
  ...
  `pumpRootMailbox`'s existing `withLock` (no nested locks).
...
**Status:** fixed 2026-09-01 (`d3ad4da`; REJECT F1-F13 → full correction round → APPROVE with operational proof: 288MB live trace auto-rotated to 313KB during review, INV1 verified end-to-end 28ms). **Priority:** P1. **Source:** user request 2026-08-31 + root trace-debt review (1.16M events / 256MB in one session; analysis is ad-hoc grep today).
...
**Status:** proposed. **Priority:** P0. **Source:** live incident 2026-08-31 09:00 (user-observed: root actively working yet nudged 3× in 15s right after goal replace).
...
3. **Vacuous idle predicate.** "All 0 non-root agents idle" is trivially true after a prune/stop with no live workers — nudges fire forever with nobody to nudge about. Fix: when effective non-root agent count is 0, the goal nudge should hold (nothing can advance; surfacing as spawn-suggestion instead, if at all).
...
`message.inject` during an active turn lands in the input queue and is only consumed at the START of the next turn. For normal coordination this is fine; for urgent control traffic (process changes, STOP/redirect, sequencing holds) it is a 23-minute hole. The root's only escalation today is raw tmux Escape (external to the engine, not durable, not traced).
...
- Worker-lane harness used (single-session root-mode lane cannot fire `pi.on("input")` mid-turn because `pumpRootMailbox` uses `pi.sendMessage`, not stdin — R10 documented design caveat)
...
**Source:** R10 synthesis. Rework cycle (review rejected → fix → test) re-opens fix+test but NOT the review node (terminal after rejected) — root must force-reopen. Graph should re-arm review automatically when a rejected-outcome review's downstream rework completes.

### Row R10-3 — Root pre-patch pump shapes after commits — P1

**Source:** R10 synthesis (live: vacuous-idle nudges fired 09:00→14:03 from an root running pre-b4d0f88 code). Policy + convention needed: commits touching pump/hooks behavior should trigger root-session restart (or hot-reload notice); commit messages should flag "root restart advised".
...
**Source:** R10 synthesis. Graph-advance nudges cannot distinguish "unassigned because forgotten" from "unassigned because orchestrated sequencing" (fired 4× on Issue 81/86 holds today). Add node deferReason or root-declared queue-intent so the checker skips deliberately held nodes.
...
- **H4** (P1): GC stop of an agent holding an open assignment is silent (no activeTaskIds in payload, no root nudge) — goal nudge stays suppressed by the orphaned node → potential idle-lock with buried diagnostics.
...
**Source:** Live incident 2026-09-01 (goal-1788166821408). For ~12h the goal pump emitted `goal.nudge.suppressed_by_active_task` on nearly every tick (1402 events in hour 00, 1258 in hour 01) because node `implement` of the Issue 83 task stayed `assigned` to an agent that had settled mid-work. The root received NO nudge — last actual nudge 08-31 14:03 — because Issue 85's suppression assumes assigned==in-flight==will-close, with NO stale threshold to lift the suppression. The 83a stale-open scan (the feature built for exactly this shape) did not save the loop for two compounding reasons: (1) the root pump was pre-patch (R10-3 restart debt), and (2) even post-patch, 83a's surfacing is TRACE-ONLY by accepted deviation — the dropped root-mailbox-nudge variant is precisely what this incident shows to be load-bearing: traces are not read by the root in real time and do not interact with goal suppression. Fix: when the stale-open scan surfaces a node, it must ALSO lift the goal-suppression for that task (or emit the deferred mailbox nudge) so the root gets woken exactly when an in-flight assignment is provably dead. Counting assertion per R10-1 (nudge fired exactly once per stale surface, not per tick).

### Row R11-2 — Node re-open doesn't hold the task open / issue fresh attempt — P1 (FIXED same-day, commit c1748f3)

**Source:** Same incident. Root force-reopening a single node after task closure: (a) the pump's closure roll-up re-derived `task.status=done` from the other 5/6 terminal nodes and re-closed the task, re-triggering the `task_closed` delivery fence on the new assignment; (b) force-setting node status to `assigned` directly did not mint a fresh attempt token, so the assignee's `swarm_update_task` calls hit `ATTEMPT_NOT_ACTIVE` and the agent (correctly) refused to act on a task it couldn't close — then settled with the node open, feeding R11-1. Graph mechanics need an root re-open path that (1) pins task.status open until the re-opened node re-closes, (2) requires going through the standard assign path to mint an attempt.
...
1. **Idle-lock detector**: active goal + all workers idle + a node `assigned`/`in_progress` with `lastActivityAt` older than threshold → append priority-high mail to the root mailbox (reuses the Issue 86 high-priority interrupt machinery to abort the root's idle turn and wake it).
...
**Sequencing note**: this is the one roadmap row that does NOT require a restart-gated pump change — it can land and protect immediately, even while the root runs pre-patch code.

### Row R11-4 — Root leader lease never probes pid liveness — P1 (FIXED same-day)

**Source:** User-reported (2026-09-01, other project): after closing the old root pane, a new pane cannot register as root — "Root already active on pid N" — indefinitely. Root cause (identity.ts readRootLeader): the deny logic consulted ONLY heartbeat age (60s TTL), never whether the leader pid still exists. Two shapes: (a) fresh death blocks claims up to 60s; (b) orphaned leader (pid survives pane close, pump keeps heartbeating) blocks FOREVER — the reported bug. Fix: readRootLeader probes `kill(pid, 0)`; a dead pid is stale immediately regardless of heartbeat freshness (EPERM treated as alive — pid exists but owned by another user). Claim/heartbeat/adopt paths all inherit the corrected staleness. Tests: multi-root R1-R4 (RED captured pre-fix: dead pid + fresh heartbeat → denied); T1/T3/T9 updated to seed real live pids because fabricated pids now correctly read as dead (the old tests encoded the no-probe assumption). 28/0. PID-reuse window noted in code comment (benign: reused pid reports alive → old TTL behavior).

### Row R11-5 — Fresh assignment to stopped agent fenced by nudge-staleness branch — P1 (FIXED same-day, commit c7c152c)

**Source:** live incident 2026-09-01 08:25 (Issue 84 kickoff; 5th idle-lock of the day). swarm_assign_task's defense-in-depth fence check (checkStallNotificationStale branch 6: agent stopped/unhealthy past grace) fired on a FRESH mint because (a) the assignee record was legitimately `stopped` — the root restarts the pane right after assigning — and (b) assignmentAge was computed from the PREVIOUS task's canonId. The canonical assignment message was replaced by a "...assignment FENCED / no canonical assignment message was sent" notice; the worker tried to self-assign (ROOT_AUTHORITY_REQUIRED) and settled idle with the node open. Fix: opts.freshAssignment skips branch (6) on the assign path (nudge paths unchanged). Repro suite assign-fresh-not-fenced.test.mjs (RED pre-fix verbatim: fenced reason=agent_stopped).

### Row R12 — Task-close sweep must not kill shared worker pool — P0 (FIXED 2026-09-01)

**Source:** Live incident 2026-09-01T12:51:50. `task-202609011740-issue-86-urgent-midturn` closed → `sweepTaskWorkersLocked` killed `fs-implementer`, `r80-tester`, `r80-reviewer`, `r10-analyst` — all four `releaseReason === "sole_active_task"`, all four `spawnedForTaskId === null` (shared pool), all four `leaseValidAtSweep === false`. ZERO non-root workers survived. Two earlier same-shape incidents: **2026-09-01T10:38:06** (3 workers, `task-202609010900-issue-84-audit-tooling`) and **2026-09-01T13:05:19** (`r10-analyst`, `task-202609011955-issue-81-goal-clear-verify`). Same root cause across all three — the engine killed workers inside what looked like normal task closure.
...
**Companion fix:** on a sweep that **transitions the effective live non-root pool from ≥1 to 0**, emit exactly one priority-high root nudge (mailbox + Issue 86 interrupt machinery) so the root wakes up and either re-spawns workers or downgrades the goal. `0 → 0` and `≥1 → ≥1` transitions do NOT nudge. Idempotent within a sweep call (a re-invocation finds `stopped.length === 0` and returns at the guard, naturally suppressing duplicate nudges). New trace constant `pool.depleted_nudge` and idempotency key `pool_depleted:<taskId>` make replays safe.
...
- `extensions/mock-llm/fixtures/r12-shared-pool-mass-sweep.jsonl` — scripted root lane: open mailbox → close task-r12-a → verify shared-pool workers preserved + exactly 1 priority-high nudge present → settle.
...
### Row R13 — High-priority root nudges must surface when tmux target is unknown — P0 (FIXED 2026-09-01)

**Source:** Live trace sequence 2026-09-01T13:10:27. `stale_open_surfaced` (task `task-202609012010-r12-shared-pool-mass-sweep`, node `implement`) → `staleOpenNudgeLocked` delivered a priority-high STALE-OPEN nudge to the root mailbox → `message.deliver.mailbox_only` logged `delivered: true` → then `notification.stale.suppressed site=root_pump.surface reason=agent_busy evidence=[effective-agent-set-not-idle]` from reconcile.ts:1665 → ZERO `pi.sendMessage` calls at the reconcile.ts:1763-1773 boundary → the user never saw the safety nudge even though the mailbox entry masqueraded as success.

**Root cause (isolated, code-located):** The root pseudo-agent has `tmuxTarget === "unknown"` by design (identity.ts:69). `deliverMessageLocked` short-circuits to mailbox-only for unknown target (mailbox.ts:200-203) — this is correct (durable append is the intended side effect). But `pumpRootMailbox` (reconcile.ts) then surfaces that durable message through `staleSurfaceReason`, which returns `{stale: true, reason: "agent_busy"}` whenever a worker is `tool_running` (or any non-idle effective-agent condition) — even for priority-high safety nudges. The pump suppresses the message, never calls `pi.sendMessage`, and the durable entry stays in `consumerReceipts` forever without a user surface. Four "appears to work" signals conspired to hide it: `mailbox_only` returns `delivered: true`, `stale_open.nudge_emitted` fires after delivery, the message persists durably, and the next pump writes a `notification.batch.suppressed total:0`.

**Fix:** At the surface-plan suppression site (reconcile.ts:1665), bypass the busy-suppression gate ONLY when the message is `priority === "high"` AND the recipient is the root pseudo-agent with `tmuxTarget === "unknown"` AND the stale reason is `agent_busy`. The nudge then falls through to the coalescing + `pi.sendMessage` path. Normal-priority traffic still respects the gate — the `goal.nudge.suppressed_by_active_task` storm from the R10 incident (roadmap.md:937) cannot regress. The bypass also handles a critical data-model subtlety: priority is stored on the **mailbox JSONL entry** (`item.msg`), NOT on the `st.messages` record (`item.rec`) because `upsertMessageRecord` does not persist `priority` — the fix reads priority from the mailbox entry with a record fallback. Emits a new `notification.surface.bypass_high_unknown_target` trace at the bypass site for observability.

**Acceptance evidence:**
- `extensions/swarm/r13-root-unknown-target.test.mjs` — RED→GREEN, 21/0. R13-S1 (priority-high + unknown-target + busy → exactly 1 `pi.sendMessage`, boundary counter at the reconcile.ts:1763-1773 loop; bypass trace fires; no stale suppression; mailbox_only durable append preserved). R13-S2 (replay → no duplicate surface, R10-1 no-duplicate guard via consumerReceipts). R13-S3 (normal-priority nudge → 0 sendMessages, `agent_busy` suppression preserved — no regression). R13-S4 (priority-high to known-target worker → not via root surface).
- `extensions/mock-llm/fixtures/r13-root-unknown-target.jsonl` — 3 scripted turns (open mailbox baseline → verify surface → settle).
- tmux lane `r13-validate:r13.0` ran with `pi -ne --provider mock-llm --model r13-root-unknown-target -e ./extensions/mock-llm -e ./extensions/swarm`; snapshot at `tmux-snapshots/r13-root-unknown-target-lane.txt`; transcripts at `.pi/mock-llm/transcripts/r13-orchestrator-unknown-target/`.
...
**Sequencing:** No restart-gated pump change required — the fix rides the existing per-tick pump surface path. The env override path and durable mailbox semantics are untouched. This closes the observability gap R12's pool-depletion nudge introduced: a depleting close now both emits the nudge AND ensures the root actually sees it.

### Row R13 P1 — R13 bypass must also gate on referenced task liveness — P0 (FIXED 2026-09-02)

**Source:** Live trace sequence 2026-09-02. After R13 P0 was deployed (priority-high unknown-target root nudges now surface via the busy-bypass), pre-R13 stale-open nudges that were durably enqueued on 2026-09-01 (when their tasks/nodes were still live) began re-surfacing to the root lane on 2026-09-02 after those tasks/nodes closed overnight. The bypass converted a moot historical alert into a fresh "act now" message — exactly the wrong behavior.

**Root cause (isolated, code-located):** The R13 P0 bypass at reconcile.ts:1665 fired whenever `priority=high AND unknown-target root AND reason=agent_busy` — it did NOT check whether the referenced task/node was still live. Two reinforcing causes:
...
2. `isActionableRootMessage` parsed only from `conversationId`, not from `idempotencyKey` — production `staleOpenNudgeLocked` (taskgraph.ts:1461) does NOT set a conversationId, only an idempotencyKey, so the predicate could not gate on liveness for those messages at all.
...
**Acceptance evidence:**
- `extensions/swarm/r13-root-unknown-target.test.mjs` — RED→GREEN for both P0 (S1/S2/S3/S4) and P1 (S7). 24/24 PASS.
...
### Row R15 — Normal root result must NOT promise bounded surface — P0 (FIXED 2026-09-02)

**Source:** Live trace sequence 2026-09-01. `swarm_send_message` (extensions/swarm/src/tools/messages.ts:42-48) returned the literal text `"its pump surfaces mailbox messages within ~5s"` to workers reporting completion via `to=root`, even though the pump defers while the root is busy (reconcile.ts:1617-1626) and the busy-suppression at reconcile.ts:1665 drops the message from the surface plan entirely. Workers either waited indefinitely or escalated; either way the swarm contract was broken.
...
**Fix (R15 B1 — honest removal per plan §4):** Removed the literal `"~5s"` text from `tools/messages.ts:42-48`. Replaced with an honest durable-no-time-bound contract that names the legitimate surface paths (root's own `agent_settled` or its next idle watchdog tick). Did NOT alter mailbox append, normal busy suppression, or reconcile behavior. Did NOT change `mailbox.ts:200-203` (mailbox-only short-circuit), `reconcile.ts:1617-1626` (busy-defer), or `reconcile.ts:1665` (busy-suppression site).

**Acceptance evidence:**
- `extensions/swarm/r15-normal-root-result.test.mjs` — RED→GREEN, 15/0 PASS. R15-S1 (priority-normal unknown-target + busy: 0 pi.sendMessage + literal `"~5s"` ABSENT from real `swarm_send_message` tool output), R15-S2 (idle-root explicit `agent_settled`: 1 pi.sendMessage, non-regression), R15-S3 (replay guard: no dup), R15-S4 (R13 high-priority bypass still works: 1 pi.sendMessage), R15-S5 (durable mailbox semantics intact: mailboxOnlyCount=1, deliver.mailbox_only trace), R15-S6 (R13 bypass MUST NOT fire for normal-priority: bypass trace absent).
...
**Sequencing:** Minimal source change in `tools/messages.ts:42-48` only. No restart-gated pump change. The root pump, mailbox delivery, busy-defer, busy-suppression, and R13 bypass paths are all untouched.
...
**Pre-existing failures NOT caused by R16 (verified via git stash):** `model-routing.test.mjs` (1 failure: `fast model -> openai preset` — env-dependent), `root-wake.test.mjs` (3 failures: `notification.backfill.receipts_written` + `notification.coalesced.suppressed` — unrelated to goal/resolve semantics). Both reproduce on master HEAD without R16 changes.

**Sequencing:** Fix A (ack-vs-resolve gating) + Fix B (vacuous-branch writeState + state.ts back-fill) + Fix C (action-oriented body) landed together in a single commit. The root MUST /reload to pick up Fix A's reset semantics; Fix B's persistence gap fix is observable on the first pump tick after /reload; Fix C's body change is observable on the next user-origin escalation. Pre-existing R14 dedupe + cooldown invariants hold (verified by R14 test suite green).
...
- `extensions/mock-llm/fixtures/r19-goal-graph-deadlock.jsonl` — root-mid-turn scripted scenario; floor fires at the right interval without a 3h wait; transcript at `.pi/mock-llm/transcripts/r19-goal-graph-deadlock/`. (Follow-up R-row: R19-FU1 — author this fixture.)
...
**Sequencing:** Fix A + Fix B in one commit (coupled — Fix B without Fix A is useless; Fix A without Fix B is a global firehose on every actionable LIVE task). The root MUST /reload to pick up the new floor semantics; observable on the next pump tick after /reload.

### Row R14 — Active user-origin goal must escalate when worker pool is empty — P0 (FIXED 2026-09-02)

**Source:** Live trace sequence 2026-09-01. `goal-1788266039522-6eae40`, origin=user, nudgeIntervalMs=5000. 7_278 `goal.nudge.held_no_live_workers` traces fired over ~16h at exactly 5s cadence. ZERO root-bound recovery nudges — the pump is in a deadlock state: the user sees nothing, the operator sees only the spam trace, the goal never escalates.
...
3. **No escalation path** (`reconcile.ts:520` return). The function returned `{emitted:false,reason:"no_live_workers"}` with no surface. Fix C adds a bounded user/system/batch-origin escalation: ONE high-priority root-bound nudge per `NOTIFY_DEFAULT_COOLDOWN_MS` (5min), durably enqueued via the existing R13 P0 surface.
...
**Source:** Live trace sequence 2026-09-01/02. The "settled idle with open assignment" pattern was observed 5+ times in a single session: r16-implementer, r19-implementer, r19-reviewer, twice on r19 commit-close. The mechanism is consistent — the worker does real work (writes artifacts, runs tests), then `agent_settled` fires WITHOUT a call to `swarm_update_task` (close) + `swarm_send_message replyTo` (link the verified result). The root's recovery ladder (probe pane → directive → restart → force-close) gets stuck on step 1: by the time the root notices, the agent pane has long settled and the worker won't see the directive without a wake-up.

A second, related problem: the root repeatedly misread `swarm_agent_status`'s 12 fields and concluded "busy" when the agent was actually `response_missing`. Simplifying the status is part of the same R-row.

**Two coupled fixes:**

1. **Artifact-progress self-nudge** (`reconcile.ts:evaluateArtifactProgressNudgeLocked`, wired at the pump-loop site alongside `evaluateIdleGoalNudgeLocked`): every pump tick, scan `node.allowedFiles ?? task.allowedFiles`, take max mtime (capped at 50 files/node). When the max mtime exceeds `max(lastProgressAt, artifactProgressNudgeAt) + ARTIFACT_PROGRESS_GRACE_MS` AND the agent's `lastToolAt` is older than `ARTIFACT_PROGRESS_ACTIVE_AGENT_SKIP_MS` (60s), deliver a high-priority, action-oriented nudge to the AGENT itself (not the root) naming the exact close-action triple:
   ...
   Backoff: `ARTIFACT_PROGRESS_NUDGE_BACKOFF_MS` (default 5min) per node. Cap: `ARTIFACT_PROGRESS_NUDGE_CAP` (default 3) nudges, then a one-line root escalation (`worker.artifact_progress_cap_exceeded` trace + durable high-priority mailbox message).
...
- Preserved: R19/R16/R14 invariants, goal evaluator, graph-stall Row 75 leg, stale-open root nudge, all existing pump phases.
...
- `extensions/swarm/idle-nudge.test.mjs` — R22 section: S1 unit (goal key NOT surface-stale with post-emission busy worker), S2 real-boundary assertions via `pumpRootMailbox` (`pi.sendMessage` ≥1 at the real boundary, coalescing 3→1, replay dedupe), controls C1 (taskKey busy suppression kept) / C2 (epoch leg kept) / C3 (live-graph leg kept)
...
- Mock-llm BEFORE/AFTER lanes: BEFORE = 3× `notification.stale.suppressed reason=agent_busy` + escalated-but-empty (`artifacts/lanes/goal-nudge-surface-starve/traces-before/pump-lines.txt`); AFTER = `mailbox.root_pump count:1`, zero suppressions, coalesced 3→1, root acked the surfaced nudge (`traces-after/pump-lines.txt`, `transcripts/`, TUI pane `tmux-snapshots/r22-goal-nudge-surface-starve/pane-after.txt`)
- Full regression green: all named suites pass (idle-nudge 81, swarm-goal 67, stale-open-nudge 6, R21 5, R20 20+16, R19 22, R16 25, R14 14, R13 24, R12 21+36, CT phase-2 38); the only failing suites repo-wide (root-wake 3, model-routing 1, minimal-protocol-authoritative 18) fail IDENTICALLY on pre-fix code (verified via stash) and are pre-existing baseline failures outside R22 scope

**Boundary counters:**
- `C-R22-1` real-boundary surfacing: `pi.sendMessage` called ≥1 via `pumpRootMailbox` with a busy worker + queued goal nudges (RED: 0 calls)
...
**AMENDMENT (R23B 2026-09-02):** R23's original fix added the cap-branch reset (once per anchor via `r23LastEpochAnchor` memo) AND an edge-site reset inside `updateIdleEpochLocked`. The edge site never consulted the memo, so it fired on EVERY busy→idle edge while saturated, defeating MAX+backoff in real sessions where `agent_settled` re-stamps the anchor at every root turn boundary. Live storm: implementer lane 2026-09-02T15:19:06..15:21:46Z — `goal.nudge.saturation_reset_on_epoch` ×12, `goal.idle_nudge` seq 4→38, `mailbox.root_pump_stuck_escalated` ×34. R23B deletes the edge-site reset (cap branch is now the SOLE reset site) and adds a worker-breaker guard on the cap branch: `idleState.lastEpochBusyAgents?.some(id => id !== "root")` (stamped on every anchor-clearing busy edge, cleared on the busy→idle edge stamp). Storm-shape regression test added in `extensions/swarm/idle-nudge.test.mjs` R23B section (12 assertions: cap-branch reset, 3 emissions, ≥2 churn edges rejected, counter preserved at MAX, backoff engaged). Types formalization: `r23LastEpochAnchor?: string` AND `lastEpochBusyAgents?: string[]` added to `SwarmIdleNudgeState` in `extensions/swarm/src/types.ts` (the R23 cast is gone). R23B task: `task-202609022240-r23b-reset-storm-rework`.

**AMENDMENT (R23C 2026-09-03):** even with the R23B edge-site deletion + worker-breaker guard, the storm rerouted through the cap branch: `hooks.ts turn_start` clears the anchor bypassing `updateIdleEpochLocked` (the root's busy edge at every turn boundary) WITHOUT stamping `lastEpochBusyAgents`; the cap branch's `breaker = undefined` → absent→reset legacy default fires the reset on every root turn. Live evidence (tester-turnstart-probe.mjs vs the R23B-only tree): 2 resets, 4 emissions, seq 4→7 across 5 turn cycles. R23C anchors provenance at every clear site: (1) `hooks.ts turn_start` now stamps `lastEpochBusyAgents = ["root"]` before clearing; (2) `reconcile.ts` mint branch now PRESERVES `lastEpochBusyAgents` instead of clearing it (provenance must survive from the clear site to the next atCap eval). R23C task: `task-202609030005-r23c-memo-mint-rework`. New R23C section in `extensions/swarm/idle-nudge.test.mjs` (10 assertions at runtime — 2 backoff drains, 5 in-loop orbit rejections, 3 bounds — source has 6 assertion sites; invokes the REAL `hooks.ts turn_start` handler via `registerSwarmHooks` mock pi — not an inline replica — so removing the stamp from hooks.ts makes the test go RED; blindness control both ways verified). Lane launcher `goal-nudge-backoff-epoch-rearm/launch.sh` re-copied under R23C artifacts with `r23LastEpochAnchor` seeded (production-mint shape). Known corner (accepted, documented): a real worker break followed by turn-start-only churn can carry a stale `["worker-a"]` stamp into one churn epoch → at most ONE wrong-cause reset per genuine worker break; bounded, not a storm. The `turn_start` handler write crosses a Pi-runtime boundary (durable state write from a lifecycle hook); see `pi-runtime-contract.md §10 F15` row (AMENDED 2026-09-03 R23C).

**Source:** Live incident 2026-09-02T14:44:37–14:45:17Z. Goal `goal-1788350610025-7efafe` (user-origin) sat at `consecutiveNoResolveNudges === MAX_CONSECUTIVE_NUDGES_DEFAULT` with `nudgeSeq=3` while a NEW all-idle epoch (anchor `idleNudgeState.allIdleSinceAt = 14:45:02Z`, strictly after the legacy nudges `msg-1788350616129-691b4e7c` / `-0aea3216` / `-c6f752b8`) was already running. `backoffTicksRemaining` cycled `2→1→0→re-arm 2` on every eligible tick; ZERO fresh `goal.idle_nudge` traces; ZERO `pi.sendMessage` at C3; the root LLM never saw a nudge. Root cause: the cap+backoff state is goal-scoped and epoch-blind — the prior epoch's nudges were already surface-invalidated (`idle_epoch_advanced`, R21) and no `turn_end` resolve could fire while the floor was starved (the cap loop emits nothing, so there was no turn to resolve), yet the saturation carried into every fresh epoch (structurally the R16 starvation shape at the cap layer). RED artifact: `r23-goal-backoff-epoch-starvation.red.mjs` (15/15 RED-shape, run before the fix).
...
- `extensions/mock-llm/fixtures/goal-nudge-backoff-epoch-rearm.jsonl` + launch lane — Pattern-2 seeded saturated-goal snapshot; root verifies + acks
...
- Suite lanes: idle-nudge **111/0**, swarm-goal **67/0**, stale-open-nudge **6/0**, R21 **5/0**, R16 **25/0**, R19 **22/0**, R14 **14/0**, R13 **24/0**, mock-llm selftest **70 models PASS**; repo-wide failures unchanged vs pre-fix baseline (root-wake 3 / model-routing 1 / minimal-protocol-authoritative 18 — stash-verified identical)
- Mock-llm lane: `artifacts/lanes/goal-nudge-backoff-epoch-rearm/{traces/pump-and-nudge-lines.txt, transcripts/}` + `tmux-snapshots/r23-goal-backoff-epoch-starvation/pane-after.txt` — C1 (`goal.nudge.saturation_reset_on_epoch` + `goal.idle_nudge` seq:4 `consecutiveCount:1` + durable `goal:goal-r23-fixture:nudge:idle-streak:4` record), C3 (`mailbox.root_pump count:1` → real send), C4 (`notification.stale.suppressed` only for the 3 LEGACY nudges via `idle_epoch_advanced` — the fresh nudge surfaced with zero suppressions), C5 (provider transcripts record `[toolcall:swarm_check_mailbox]` + `[toolcall:swarm_ack_message]` in the root turn)
...
### Row R24 — Worker result messages are durably delivered but never visibly surfaced without manual `swarm_check_mailbox` — P0 (FIXED 2026-09-03)

**Source:** Live incident 2026-09-02T15:26:06.708Z: `msg-1788362766708-64f55b39` (R23 implement-done result message from `r22-implementer` to `root`) was durably enqueued at 15:26:06.714Z (C1 + C2 `message.deliver.mailbox_only`; root has no tmux pane by design, R15/F3), then durably classified `node_terminal` by the per-tick actionability filter (`isActionableRootMessage`) at 15:26:06.913Z and again at 15:28:08.886Z — `notification.stale.suppressed site=root_pump.surface reason=node_terminal evidence=[node_terminal]` with ZERO `pi.sendMessage` at the swarm→Pi bridge. The root LLM never saw the result; the root's own `agent_settled` and watchdog pumps ran every 5s and kept suppressing. `mailbox.root_pump_stuck_escalated queued:3 oldestWaitMs:310277` at 15:31:01.991Z, then a manual `swarm_check_mailbox` at 15:31:09.993Z surfaced the envelope (`message.lifecycle_derived_shadow source=mailbox.surfaced stage=seen via=swarm_check_mailbox`) and the root ack'd done at 15:31:26.434Z. Sibling cases this session: my own R22 verdict and the R23B rework-result both surfaced only because I manually replied in-thread (same shape, different message class). RED artifact: `artifacts/r24-visible-surface-gap.red.mjs` (run BEFORE the fix) — C1 ✓ C2 ✓ `notification.stale.suppressed reason=node_terminal` ✓ C3/C4 (`pi.sendMessage`) === 0 ✓; CONTROL nudge on a terminal node also suppressed (gate intact, no false-positive exemption).

**Fix:** a single predicate change in `isActionableRootMessage` (the per-tick actionability filter that builds `windowMsgs` in `pumpRootMailbox`): a message whose fingerprint is `requiresAck && !requiresResponse && replyTo` set (the close-out shape — RESULT/REPLY/ASSIGNMENT messages produced by `swarm_send_message replyTo=...`) is exempted from `node_terminal` / `task_done` / `task_failed` / `task_cancelled` suppression so the recipient PM sees the outcome without manual polling. The exemption check fires BEFORE the task-terminal legs so nudges (canonical `task:<id>:node:<id>:nudge:*` idempotencyKey without `replyTo`) keep full gating. No new `pi.sendMessage` call site is introduced; the existing pump boundary counts at the real `pumpRootRealSend` counter (R10-1 boundary counter discipline preserved). The change crosses **L3 visible surface only** (a predicate flip on the existing surface decision inside the pump). L1/L2/C1/C2 (durable mailbox delivery) was always working — the gap was L3 specifically. L4/L5 (LLM consumption) is asserted only when the provider transcript proves it (the fixture's first turn ack `done` proves C5 in the live lane).

**Files touched:**
- `extensions/swarm/src/reconcile.ts` — `isActionableRootMessage` task-scoped branch: 4 new early-return predicates (`result_class_exempt_task_done/failed/cancelled/node_terminal`) gated on `isResultClass` computed ONCE at the top of the block
- `extensions/swarm/idle-nudge.test.mjs` — R24 section: 9 assertions exercising the predicate directly (result-class exempted on done/failed/cancelled/terminal-node; nudges fall through; without `replyTo` or with `requiresResponse:true` falls through to the original gates)
- `extensions/mock-llm/fixtures/root-visible-surface-gap.jsonl` + `selftest.test.mjs` registry count 70→71 — Pattern-2 seeded worker-result envelope + root ack lifecycle; second turn asserts replay dedupe (zero new `pi.sendMessage`)
- `.pi/swarm/tasks/task-202609022235-r24-orchestrator-visible-surface-gap/artifacts/lanes/orchestrator-visible-surface-gap/launch.sh` — seeds the live-incident shape (task in_progress + implement DONE + result-class msg mailbox JSONL appended) and runs `pi --provider mock-llm --model root-visible-surface-gap` in a dedicated tmux window
- `docs/swarm/pi-runtime-contract.md` — §10 F16 row (corrects the prior contract claim that terminal nodes/tasks universally suppress; the contract now distinguishes result-class from nudge messages)
- `docs/swarm/operations.md` — R24 operator subsection (field diagnosis, verify commands, what changed, known corners)

**Acceptance evidence:**
- RED artifact: `artifacts/red-pre-fix.txt` — result message: C1 ✓ C2 ✓ 1 suppression trace ✓ 0 `pi.sendMessage` ✓; CONTROL nudge: 1 suppression trace ✓ (gate intact)
- GREEN artifact: `artifacts/green-post-fix.txt` — result message: 1 `pi.sendMessage` call ✓ 0 suppression traces ✓; CONTROL nudge: 1 suppression trace ✓ (gate intact)
- Suite lanes: idle-nudge **145/0** (was 136; +9 R24 section), swarm-goal **67/0**, stale-open-nudge **6/0**, R15-normal-root-result **15/15**, R13-root-unknown-target **24/24**, mock-llm selftest **71 models PASS**; repo-wide failures unchanged vs pre-fix baseline (agent-lifecycle 1 / functional 1 — stash-verified identical to R23C baseline)
- Real mock-llm lane: `artifacts/lanes/orchestrator-visible-surface-gap/tester-traces/events.jsonl` — `mailbox.root_pump reason:session_start count:1 ids:["msg-r24-result-1"]` (C-R24-1 boundary assertion at the real `pi.sendMessage`), `message.ack status=done` (C-R24-5 LLM consumption — fixture turn 1 ack closes the lifecycle), subsequent watchdog ticks `count:0, deferred:0` (C-R24-2 replay dedupe holds)

**Boundary counters:**
- `C-R24-1` real-boundary send: `pi.sendMessage` ≥1 at the pump in the live lane (the `mailbox.root_pump reason:session_start count:1` line in the trace)
- `C-R24-2` replay dedupe: zero subsequent `pi.sendMessage` calls for the same already-surfaced msgId (verified at watchdog ticks post-ack in the live lane)
- `C-R24-3` CONTROL nudge still suppressed in the driver (gate intact — proven by the `notification.stale.suppressed reason:node_terminal` trace on the control msgId in both pre-fix and post-fix driver runs)
- `C-R24-4` predicate unit: `isActionableRootMessage` returns `result_class_exempt_node_terminal` for `requiresAck && !requiresResponse && replyTo` on a terminal node (R24-1 / R24-8 unit assertions); returns `node_terminal` for the same fingerprint with `requiresResponse:true` or no `replyTo` (R24-6 / R24-7); returns `node_terminal` for nudges (R24-2 / R24-9)
- `C-R24-5` LLM consumption (C5): asserted from the mock-llm provider transcript — the root's first turn after the surface includes the surfaced msg and runs `swarm_ack_message status=done`, proving the LLM consumed the surfaced result

**Sequencing:** R24 touches only the surface-time actionability filter for task-scoped messages. R13 P0 bypass, R13 P1 liveness gate, R15 normal-priority surface, R21 `idle_epoch_advanced`, R22 worker-busy surface rule, R23B/R23C storm guard (idle-nudge 145/0 incl. all R23 sections), the active-task emission gate, and delivery machinery are all untouched. The `result_class_exempt_*` reason codes are NEW observation surfaces in the suppression census — they don't change existing counts (acked / dead_letter / superseded / task_done / task_failed / task_cancelled / node_terminal / node_reassigned / task_missing / node_missing / wrong_recipient / retrigger_budget_exhausted / informational_already_consumed); the predicate returns `ok:true` for result-class messages so they bypass the census entirely (they're actionable, not suppressed).
