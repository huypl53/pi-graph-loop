...
st.messages = {
	...
	m3: {
		...
		requiresAck: true,
		ackedAt: ts(-2000),
		lastAck: { by: "worker", status: "done", at: ts(-2000) },
	},
	m4: {
		id: "m4",
		from: "root",
		to: "worker",
		status: "injected",
		createdAt: ts(-60 * 60 * 1000),
		updatedAt: ts(-60 * 60 * 1000),
		attempts: 1,
		requiresAck: true,
	},
};
...
ok(
	"event filter matches agent/task",
	(await readAuditEvents(p, { task: taskId, limit: 5 })).events.some((e) => e.detail.taskId === taskId),
);

const timeline = await auditTimeline(p, "m1", {});
ok(
	"timeline contains ordered stages",
	timeline.timeline.stages
		.map((s) => s.stage)
		.join(",")
		.includes("enqueue") && timeline.timeline.stages.map((s) => s.stage).includes("response_verified"),
);
ok("timeline duration is measured", timeline.durationMs >= 0);

const probes = __test;
ok(
	"probe P1 flags old actionable",
	probes.probeP1(st, 1).some((r) => r.messageId === "m4"),
);
ok(
	"probe P2 lists dead_letter",
	probes.probeP2(st).some((r) => r.messageId === "m2"),
);
ok(
	"probe P3 coalesces epochs",
	probes.probeP3([
		{ event: "mailbox.root_pump_stuck_escalated", ts: ts(1), oldestWaitMs: 1 },
		{ event: "mailbox.root_pump_stuck_escalated", ts: ts(2), oldestWaitMs: 3 },
	]).length === 1,
);
ok(
	"probe P4 flags burst",
	probes.probeP4([
		{ event: "goal.nudge.emitted", ts: ts(1), detail: { goalId: "goal-1" } },
		{ event: "goal.nudge.emitted", ts: ts(2), detail: { goalId: "goal-1" } },
		{ event: "goal.nudge.emitted", ts: ts(3), detail: { goalId: "goal-1" } },
		{ event: "goal.nudge.emitted", ts: ts(4), detail: { goalId: "goal-1" } },
	]).length === 1,
);
ok(
	"probe P4 negative 2 in window stays quiet",
	probes.probeP4([
		{ event: "goal.nudge.emitted", ts: ts(1), detail: { goalId: "goal-2" } },
		{ event: "goal.nudge.emitted", ts: ts(2), detail: { goalId: "goal-2" } },
	]).length === 0,
);
ok(
	"probe P4 negative 3 spread beyond window stays quiet",
	probes.probeP4([
		{ event: "goal.nudge.emitted", ts: ts(1), detail: { goalId: "goal-3" } },
		{ event: "goal.nudge.emitted", ts: ts(61_001), detail: { goalId: "goal-3" } },
		{ event: "goal.nudge.emitted", ts: ts(122_002), detail: { goalId: "goal-3" } },
	]).length === 0,
);

const inv = await checkInvariants(p, st);
ok(
	"invariants flag seeded violations",
	inv.invariants.some((i) => i.violated && i.invariant === "INV1") &&
		inv.invariants.some((i) => i.violated && i.invariant === "INV2") &&
		inv.invariants.some((i) => i.violated && i.invariant === "INV3"),
);
