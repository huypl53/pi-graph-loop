...
ok(
	"backup names are timestamped+unique",
	stateBackups.every((f) => /^swarm-state\.json\.\d+-\d{6}-[0-9a-f]+$/.test(f)),
);
...
for (let i = 1; i <= 6; i++) {
	...
	if (!task) {
		task = {
			...
			nodes: { a: { status: "pending", role: "w", dependsOn: [], messageIds: [], attempts: 0 } },
			edges: [],
			handoffs: [],
			gates: {},
			editLocks: {},
