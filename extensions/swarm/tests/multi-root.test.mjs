...
const expectErr = async (fn, code) => {
	try {
		...
		return String(e?.message || e).includes(code);
	}
};
...
const mkAgent = (id, roleKind = "worker") => ({
	id,
	role: id,
	roleKind,
	capabilities: [],
...
const setLeaderStale = (pid, staleMs = 120000) => {
	...
	st.rootLeader = {
		...
		sessionStartedAt: new Date().toISOString(),
		claimedAt: new Date().toISOString(),
		lastHeartbeatAt: new Date(Date.now() - staleMs).toISOString(),
		agentRecordId: "root",
	};
	writeFileSync(statePath, JSON.stringify(st, null, 2));
};
...
let denied = false;
try {
	(await importIdentity()).heartbeatRootLeader(st, Date.now(), 444, "test");
} catch (e) {
	denied = String(e?.message || e).includes("ROOT_LEADER_DENIED");
}
...
ok(
	"T5 update force denied worker",
	await expectErr(() => call("swarm_update_task", { taskId: "t5", nodeId: "plan", force: true }), "FORCE_FORBIDDEN"),
);

// T6: update_task(cancelTask=true) denied worker
ok(
	"T6 update cancel denied worker",
	await expectErr(() => call("swarm_update_task", { taskId: "t5", nodeId: "plan", force: true, cancelTask: true }), "CANCEL_FORBIDDEN"),
);
...
ok(
	"T9 reconcile mark denied non-leader",
	await expectErr(() => call("swarm_reconcile", { mark: true, dryRun: false }), "ROOT_LEADER_DENIED"),
);
try {
	t9Child.kill("SIGKILL");
} catch {
	/* already gone */
}
...
process.env.PI_SWARM_AGENT_ID = "worker";
ok(
	"T11 assign denied live leader",
	await expectErr(() => call("swarm_assign_task", { taskId: "t10", nodeId: "a" }), "ROOT_AUTHORITY_REQUIRED"),
);
...
ok(
	"T12 update unassigned node = claim (Issue 24.a)",
	await (async () => {
		try {
			await call("swarm_update_task", { taskId: "t10", nodeId: "a", status: "done" });
			return true;
		} catch (e) {
			return String(e?.message || e).includes("NODE_ASSIGNEE_MISMATCH");
		}
	})(),
);
...
const st17 = JSON.parse(readFileSync(statePath, "utf8"));
st17.rootLeader = {
	pid: 777,
	sessionStartedAt: new Date().toISOString(),
	claimedAt: new Date().toISOString(),
...
const st18 = JSON.parse(readFileSync(statePath, "utf8"));
delete st18.rootLeader;
const claim18 = (await importIdentity()).claimRootLeader(st18, Date.now(), 999);
ok("T18 vacant claim succeeds", claim18.kind === "claimed" && st18.rootLeader.pid === 999);

// T19: update task by root allowed
process.env.PI_SWARM_AGENT_ID = "root";
ok(
	"T19 root update allowed",
	text(await call("swarm_update_task", { taskId: "t10", nodeId: "a", force: true, status: "done", outcome: "ok" })).includes(
		"Updated node",
	),
);

// T20: stale leader helper remains replaceable after time passes
const st20 = JSON.parse(readFileSync(statePath, "utf8"));
st20.rootLeader = {
	pid: 1111,
	sessionStartedAt: new Date().toISOString(),
	claimedAt: new Date().toISOString(),
...
process.env.PI_SWARM_IS_ROOT = "";
ok(
	"T21 worker create denied on vacant state",
	await expectErr(
		() =>
