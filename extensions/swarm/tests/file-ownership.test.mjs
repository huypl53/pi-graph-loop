...
ok(
	"scope: node-explicit",
	JSON.stringify(tg.resolveNodeScope(mkTask({ a: { allowedFiles: ["x.ts"] } }), "a")) ===
		JSON.stringify({ source: "node-explicit", files: ["x.ts"] }),
);
ok(
	"scope: node-inherited",
	JSON.stringify(tg.resolveNodeScope(mkTask({ a: { allowedFilesFrom: "b" }, b: { allowedFiles: ["y.ts"] } }), "a")) ===
		JSON.stringify({ source: "node-inherited", sourceNodeId: "b", files: ["y.ts"] }),
);
ok(
	"scope: task-default",
	JSON.stringify(tg.resolveNodeScope(mkTask({ a: {} }, ["z.ts"]), "a")) === JSON.stringify({ source: "task-default", files: ["z.ts"] }),
);
ok(
	"scope: cycle unresolved",
	"unresolved" in tg.resolveNodeScope(mkTask({ a: { allowedFilesFrom: "b" }, b: { allowedFilesFrom: "a" } }), "a"),
);
...
await call("swarm_create_task", {
	title: "Ownership overlap same task",
	goal: "g",
	taskId: "task-own-st",
	start: "n1",
	nodes: {
...
const conflictErr = await expectErrorCode(
	"swarm_assign_task",
	{ taskId: "task-own-st", nodeId: "n2", agentId: "worker-b" },
	"ACTIVE_SCOPE_CONFLICT",
);
ok(
	"conflict payload has conflicting fields",
	conflictErr &&
		conflictErr.conflictingNodeId === "n1" &&
		conflictErr.conflictingTaskId === "task-own-st" &&
		conflictErr.conflictingAssignee === "worker-a" &&
		conflictErr.conflictingAttemptId?.startsWith("attempt-"),
);
ok(
	"conflict payload has requested scope + relation + hint",
	conflictErr &&
		Array.isArray(conflictErr.requestedScope) &&
		conflictErr.relation === "equal" &&
		typeof conflictErr.actionableHint === "string",
);
...
ok(
	"disjoint same-task nodes both assignable",
	readNode(taskA, "n1").assignee === "worker-a" && readNode(taskA, "n2").assignee === "worker-b",
);
...
await call("swarm_create_task", {
	title: "Ownership B overlap",
	goal: "g",
	taskId: "task-own-b",
	start: "m1",
	nodes: { m1: { role: "implementer", allowedFiles: ["docs/swarm/a.md"], terminal: true } },
	edges: [],
});
...
await call("swarm_create_task", {
	title: "Ownership C glob",
	goal: "g",
	taskId: "task-own-c",
	start: "m1",
	nodes: { m1: { role: "implementer", allowedFiles: ["docs/swarm/**"], terminal: true } },
	edges: [],
});
...
await call("swarm_create_task", {
	title: "Ownership D disjoint",
	goal: "g",
	taskId: "task-own-d",
	start: "m1",
	nodes: { m1: { role: "implementer", allowedFiles: ["src/other.ts"], terminal: true } },
	edges: [],
});
...
await awaitAs("worker-a", "swarm_update_task", {
	...
	status: "done",
	outcome: "done",
	attemptId: nA1.activeAttemptId,
});
const doneAttempt = readNode(taskA, "n1").attemptHistory.at(-1);
ok(
	"terminal attempt released with reason",
	doneAttempt.status === "completed" && doneAttempt.releasedAt && doneAttempt.releaseReason === "terminal",
);
...
await awaitAs("worker-a", "swarm_update_task", {
	...
	nodeId: "n1",
	status: "in_progress",
	attemptId: stNode.activeAttemptId,
});
...
ok(
	"reassign supersedes prior lease with reason",
	stHist.length === 2 &&
		stHist[0].status === "superseded" &&
		stHist[0].releaseReason === "reassign" &&
		stHist[1].status === "active" &&
		stHist[1].assignee === "worker-b",
);
...
await call("swarm_create_task", {
	title: "Ownership rw conflict",
	goal: "g",
	taskId: "task-own-rwc",
	start: "m1",
	nodes: { m1: { role: "implementer", allowedFiles: ["docs/swarm/rw.md"], terminal: true } },
	edges: [],
});
...
const legacyTask = {
	...
	allowedFiles: ["docs/swarm/legacy.md"],
	acceptanceCriteria: [],
	validationCommands: [],
	start: "n1",
	currentNodes: ["n1"],
...
ok(
	"attempt history persisted with scope + release audit",
	reloaded.nodes.n1.attemptHistory.every((a) => a.attemptId && a.status),
);
