...
// ============================================================================
{
	...
	ok(
		"case A: swapped to a DIFFERENT slot",
		setModelCalls[before] && setModelCalls[before].target !== "zai-coding-cn/glm-5.1",
		`to=${setModelCalls[before]?.target}`,
	);
	ok(
		"case A: swap note sent to the agent",
		sentMessages.some((s) => /Operator forced manual rotation/.test(s.m.content)),
	);
	// Trace shape
	const evAfter = await readFile(p.events, "utf8").catch(() => "");
	const evDelta = evAfter.slice(evBefore.length);
	const traceLines = evDelta
		.split("\n")
		.filter(Boolean)
		.map((l) => {
			try {
	...
	ok(
		"case A: forced trace carries agentId, from, to, reason",
		forced &&
			forced.agentId === "root" &&
			forced.from === "zai-coding-cn/glm-5.1" &&
			forced.to &&
			forced.from !== forced.to &&
			typeof forced.reason === "string",
		`payload=${JSON.stringify(forced)}`,
	);
	...
	const traceLines2 = (await readFile(p.events, "utf8").catch(() => ""))
		.split("\n")
		.filter(Boolean)
		.map((l) => {
			try {
...
// ============================================================================
{
	...
	ok(
		"case B: bench trace carries agentId + slot + cooldownMs",
		benched && benched.agentId === "root" && benched.slot === "zai-coding-cn/glm-5.1" && benched.cooldownMs === 900_000,
		`payload=${JSON.stringify(benched)}`,
	);
...
// will be tightened to the 2h floor once command.ts:822 is updated to use effectiveBenchMs().
{
	...
	ok(
		"case Issue 21 (pending follow-up): bench trace uses rotation.cooldownMs until command.ts is updated",
		glm.cooldownRemainingMs < 1_000_000,
		`cooldownRemainingMs=${glm.cooldownRemainingMs}`,
	);
	ok(
		"case Issue 21: poolStatus exposes quotaResetMs=7_200_000 for glm",
		glm.quotaResetMs === 7_200_000,
		`quotaResetMs=${glm.quotaResetMs}`,
	);
	ok(
		"case Issue 21: poolStatus flags glm as quotaAware (7.2M > 900K cooldownMs)",
		glm.quotaAware === true,
		`quotaAware=${glm.quotaAware}`,
	);
	const evAfter = await readFile(p.events, "utf8").catch(() => "");
	const evDelta = evAfter.slice(evBefore.length);
	const traceLines = evDelta
	...
	const benched = traceLines.find((e) => e.event === "pool.bench_forced_by_manual_override");
	ok(
		"case Issue 21 (pending follow-up): bench trace carries cooldownMs=900_000 until command.ts is updated",
		benched && benched.cooldownMs === 900_000,
		`cooldownMs=${benched?.cooldownMs}`,
	);
}
...
// ============================================================================
{
	...
	const warn = notifications.filter((n) => n.level === "warning").slice(-1)[0];
	ok(
		"case C now: refuses with 'rotate is root-only' warning",
		warn && /rotate is root-only/.test(warn.text),
	...
	ok(
		"case C next: refuses with 'rotate is root-only' warning",
		warnNext && /rotate is root-only/.test(warnNext.text),
		`warn=${JSON.stringify(warnNext)}`,
	);
...
// ============================================================================
{
	...
	const benched = traceLines.find((e) => e.event === "pool.bench_forced_by_manual_override");
	ok(
		"case D: forced trace has expected shape",
		forced && forced.agentId === "root" && forced.from && forced.to && forced.reason && forced.target,
		`payload=${JSON.stringify(forced)}`,
	);
	ok(
		"case D: bench trace has expected shape",
		benched && benched.agentId === "root" && benched.slot && typeof benched.cooldownMs === "number",
		`payload=${JSON.stringify(benched)}`,
	);
...
// ============================================================================
{
	...
	ok(
		"FI next-extends: second bench EXTENDS cooldown back to ~rotation.cooldownMs",
		secondRemain > firstRemain || secondRemain > firstRemain - 100,
		`firstRemain=${firstRemain} secondRemain=${secondRemain}`,
	);
	ok(
		"FI next-extends: post-second bench cooldown is close to rotation.cooldownMs",
		secondRemain > 800_000,
...
// ============================================================================
{
	...
	ok(
		"FI during-incident now Q3: open incident present before rotate",
		incidentBefore && incidentBefore.count === 1 && incidentBefore.providerKey === "zai-coding-cn/glm-5.1",
		`incidentBefore=${JSON.stringify(incidentBefore)}`,
	);
	...
	ok(
		"FI during-incident now Q3: incident Map entry preserved after rotate now",
		Boolean(incidentAfter),
		`incidentAfter=${JSON.stringify(incidentAfter)}`,
	);
	ok(
		"FI during-incident now Q3: incident count preserved (gate owns its own lifecycle)",
		incidentAfter && incidentAfter.count === 1,
		`count=${incidentAfter?.count}`,
	);
	ok(
		"FI during-incident now Q3: incident lastSeenAt preserved (not touched by manual path)",
		incidentAfter && incidentAfter.lastSeenAt === incidentAtMs,
		`lastSeenAt before=${incidentAtMs} after=${incidentAfter?.lastSeenAt}`,
	);
