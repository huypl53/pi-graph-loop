...
import {
	...
	FlowDialog,
	pickFlowTask,
	deriveCurrentNodeIds,
	buildStoryLine,
	buildGraphTree,
	buildHandoffLines,
	messageLifecyclePhrase,
} from "../src/flow-dialog.ts";
...
const branchTask = baseTask(branchTaskId, "Branch + rework flow", {
	...
	nodes: {
		plan: {
			...
			attempts: 1,
			outcome: "planned",
			lastActivityAt: iso(-8_000),
		},
		implement: {
			status: "done",
			role: "implementer",
			assignee: "obs-implementer",
			dependsOn: ["plan"],
			messageIds: ["msg-impl"],
			assignmentMessageId: "msg-impl",
			attempts: 1,
			outcome: "implemented",
			lastActivityAt: iso(-7_000),
		},
		review: {
			status: "assigned",
			role: "reviewer",
			assignee: "obs-reviewer",
			dependsOn: ["implement"],
			messageIds: ["msg-review"],
			assignmentMessageId: "msg-review",
			attempts: 1,
			lastActivityAt: iso(-6_000),
		},
		fix: {
			status: "blocked",
			role: "implementer",
			...
			dependsOn: ["review"],
			messageIds: ["msg-fix"],
			assignmentMessageId: "msg-fix",
			attempts: 2,
			staleAt: iso(-50_000),
			lastActivityAt: iso(-5_500),
		},
...
const blockedTask = baseTask(blockedTaskId, "Blocked task", {
	nodes: {
		...
		implement: {
			...
			dependsOn: ["plan"],
			messageIds: ["msg-block"],
			assignmentMessageId: "msg-block",
			attempts: 1,
			staleAt: iso(-70_000),
			lastActivityAt: iso(-9_000),
		},
	},
...
const beforeTask = createHash("sha256")
	.update(readFileSync(taskPaths(p, branchTaskId).taskJson))
	.digest("hex");
...
ok(
	"snapshot includes attention hints",
	data.attention.some((item) => item.hint.includes("/swarm")),
);

const tree = buildGraphTree(branchTaskState);
ok(
	"tree includes all branch task nodes",
	["plan", "implement", "review", "fix", "validate", "close"].every((id) => tree.entries.some((e) => e.nodeId === id)),
);
...
ok(
	"D5 latest non-superseded message is inline",
	buildHandoffLines(branchTaskState, branchSt).some((line) => line.edge === "review→fix" && /stuck|waiting/.test(line.text)),
);
ok(
	"D5 no-record states are explicit",
	buildHandoffLines(linearTask, branchSt).some((line) => /no handoff record/.test(line.text)),
);
ok(
	"D2 tree covers all linear nodes without truncation",
	(() => {
		const model = buildGraphTree(linearTask);
		const text = model.entries.map((entry) => entry.nodeId).join(" ");
		return Array.from({ length: 12 }, (_, i) => `node${i + 1}`).every((id) => text.includes(id));
	})(),
);
...
ok(
	"non-TUI picker falls back without custom",
	(await pickFlowTask({ cwd: scratch, mode: "print", hasUI: false, ui: { notify: () => {} } }, scratch, p)) === undefined,
);

const dialog = new FlowDialog(
	tui,
	theme,
	{ p, cwd: scratch, task: branchTaskState, tp: branchTaskFile, st: branchSt, eventLimit: 4 },
	() => {},
);
await wait();
let dialogText = dialog.render(140).join("\n");
ok(
	"default render shows graph tree and detail",
	dialogText.includes("GRAPH") && dialogText.includes("detail") && dialogText.includes("review"),
);
...
for (const W of [80, 120]) {
	...
	ok(
		`V3 render pads every line to W=${W} (straight right border)`,
		lines.every((ln) => visibleWidth(ln) <= W) &&
			lines.every((ln, i) => (i === 0 ? visibleWidth(ln) >= W - 1 : visibleWidth(ln) === W)),
	);
...
ok(
	"tree labels clean: no [planned]/[implemented]/(seen) jargon",
	!["[planned]", "[implemented]", "(seen)"].some((j) => dialog.render(140).join("\n").includes(j)),
);
...
dialog.handleInput("r");
await wait(50);
const afterTask = createHash("sha256")
	.update(readFileSync(taskPaths(p, branchTaskId).taskJson))
	.digest("hex");
