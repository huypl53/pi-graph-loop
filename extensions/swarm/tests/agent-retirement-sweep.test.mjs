...
console.log("\n[C2] sole-task worker WITH leaseKind:'reuse' + valid leaseUntil → skipped");
{
	...
	const state = makeState({
		...
		"worker-c2": makeAgent("worker-c2", {
			...
			tmuxTarget: "s:worker-c2.0",
			leaseKind: "reuse",
			leaseUntil: futureIso,
	...
	ok(
		"C2 worker-c2 in skipped as 'lease_reuse'",
		result.skipped?.some((s) => s.agentId === "worker-c2" && s.reason === "lease_reuse"),
	);
...
console.log("\n[C3] sole-task worker WITH leaseKind:'park' + valid leaseUntil → paused + parked trace");
{
	...
	const state = makeState({
		...
		"worker-c3": makeAgent("worker-c3", {
			spawnedForTaskId: taskId,
			activeTaskIds: [taskId],
			tmuxTarget: "s:worker-c3.0",
			leaseKind: "park",
			leaseUntil: futureIso,
			leaseReason: "park for inspection",
		}),
	});
	...
	ok(
		"C3 worker-c3 also in skipped as 'lease_park'",
		result.skipped?.some((s) => s.agentId === "worker-c3" && s.reason === "lease_park"),
	);
...
console.log("\n[C4] lease with EXPIRED leaseUntil → default behavior (stop)");
{
	...
	const state = makeState({
		...
		"worker-c4": makeAgent("worker-c4", {
			spawnedForTaskId: taskId,
			activeTaskIds: [taskId],
			tmuxTarget: "s:worker-c4.0",
			leaseKind: "reuse",
			leaseUntil: expiredIso,
			leaseReason: "expired",
		}),
	});
...
console.log("\n[C5] lease valid but cross-task → cross-task rule wins (skipped)");
{
	...
	const state = makeState({
		...
		"worker-c5": makeAgent("worker-c5", {
			spawnedForTaskId: taskId,
			activeTaskIds: [taskId, otherTaskId],
			tmuxTarget: "s:worker-c5.0",
			leaseKind: "reuse",
			leaseUntil: futureIso,
			leaseReason: "reuse",
		}),
	...
	ok(
		"C5 skipped as 'cross_task_active'",
		result.skipped?.some((s) => s.agentId === "worker-c5" && s.reason === "cross_task_active"),
	);
}
...
console.log("\n[C6] archived task (missing task.json) → agents with activeTaskIds:[taskId] still swept");
{
	...
	const events = await readEvents();
	ok(
		"C6 task_sweep_stopped trace emitted",
		events.some((e) => e.event === "agent.task_sweep_stopped" && e.agentId === "worker-c6"),
	);
}
...
console.log("\n[C8] idempotent re-invocation: second call is a no-op");
{
	...
	ok(
		"C8 second call: worker-c8 in skipped as 'already_stopped'",
		r2.skipped?.some((s) => s.agentId === "worker-c8" && s.reason === "already_stopped"),
		JSON.stringify(r2),
	);
...
console.log("\n[C-R11-2] live assigned/in_progress node → skipped as live_assignment_in_graph");
{
	...
	ok(
		"C-R11-2 skipped reason live_assignment_in_graph",
		result.skipped?.some((s) => s.agentId === "worker-c1" && s.reason === "live_assignment_in_graph") === true,
		JSON.stringify(result),
	);
	const finalState = await readStateFile();
	ok("C-R11-2 worker still running", finalState.agents["worker-c1"]?.status === "running");
	ok(
		"C-R11-2 activeTaskIds preserved",
		Array.isArray(finalState.agents["worker-c1"]?.activeTaskIds) && finalState.agents["worker-c1"].activeTaskIds.includes(taskId),
	);
}
