...
const sent = (e, taskId, nodeId) =>
	Object.values(e.messages).filter((r) => r.idempotencyKey === `task:${taskId}:node:${nodeId}:nudge:assign`).length;
const acked = (e, taskId, nodeId) => {
	const r = Object.values(e.messages).find((x) => x.idempotencyKey === `task:${taskId}:node:${nodeId}:nudge:assign`);
	return !!(r && r.ackedAt);
};
	...
	const t = [
		{
			...
			status: "in_progress",
			nodes: {
				p: { status: "done", assignee: "x", deps: [] },
				q: { status: "pending", assignee: null, deps: ["p"] },
			},
		},
	];
