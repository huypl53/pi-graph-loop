	...
	const pi = {
		...
		exec: async (cmd, args) => {
			...
			if (cmd === "git" && args?.[0] === "diff")
				return {
					code: 0,
					stdout: " extensions/swarm/src/tools/tasks.ts | 10 +++++-----\n 1 file changed, 5 insertions(+), 5 deletions(-)\n",
					stderr: "",
				};
		...
		() =>
			as("worker-a", false, () =>
				call(tools, "swarm_update_task", {
					...
					attestations: [
						{ claim: "Passed: attestation.test.mjs", tool: "bash", eventId: "e-nope", ts: "2026-08-29T00:00:00.000Z" },
					],
					cwd: scratch,
				}),
			),
...
// Scenario 2: cited failure event blocks Passed claim
{
	...
	await attach(
		eventsPath,
		`${JSON.stringify({ ts: "2026-08-29T10:00:00.000Z", event: "tool.executed", eid: "e-fail", tool: "bash", isError: true, cls: "error", exitCode: 1 })}\n`,
	);
	await expectReject(
		() =>
			as("worker-b", false, () =>
				call(tools, "swarm_update_task", {
					...
					note: "Passed: attestation.test.mjs",
					attestations: [
						{ claim: "Passed: attestation.test.mjs", tool: "bash", eventId: "e-fail", ts: "2026-08-29T10:00:00.000Z" },
					],
					cwd: scratch,
				}),
			),
...
// Scenario 4: ts mismatch rejected
{
	...
	await attach(
		...
		`${JSON.stringify({ ts: "2026-08-29T11:00:00.000Z", event: "tool.executed", eid: "e-ts", tool: "bash", isError: false, cls: "success", exitCode: 0 })}\n`,
	);
	await expectReject(
		() =>
			as("worker-d", false, () =>
				call(tools, "swarm_update_task", {
					...
					note: "Passed: attestation.test.mjs",
					attestations: [
						{ claim: "Passed: attestation.test.mjs", tool: "bash", eventId: "e-ts", ts: "2026-08-29T11:00:01.000Z" },
					],
					cwd: scratch,
				}),
			),
...
// Scenario 5: honest attestation passes and includes diffstat
{
	...
	await attach(
		join(scratch, ".pi/swarm/traces/events.jsonl"),
		`${JSON.stringify({ ts: "2026-08-29T12:00:00.000Z", event: "tool.executed", eid: "e-ok", tool: "bash", isError: false, cls: "success", exitCode: 0 })}\n`,
	);
	const res = await as("worker-e", false, () =>
		call(tools, "swarm_update_task", {
			taskId,
			nodeId: "only",
			status: "done",
	...
	const taskEvents = await readFile(join(scratch, `.pi/swarm/tasks/${taskId}/events.jsonl`), "utf8").then((raw) =>
		raw
			.split("\n")
			.filter(Boolean)
			.map((line) => {
	...
	ok(
		"attestation ok trace emitted",
		taskEvents.some((e) => e.event === "task.attestation.ok" && e.taskId === taskId),
	);
	ok(
		"attestation diffstat trace emitted",
		taskEvents.some((e) => e.event === "task.attestation.diffstat" && e.taskId === taskId),
	);
}
...
// Scenario 7: legacy tool+ts citation resolves
{
	...
	await attach(
		join(scratch, ".pi/swarm/traces/events.jsonl"),
		`${JSON.stringify({ ts: "2026-08-29T13:00:00.000Z", event: "tool.invoked", tool: "bash", cls: "success", durationMs: 12 })}\n`,
	);
	const res = await as("worker-g", false, () =>
		call(tools, "swarm_update_task", {
			taskId,
			nodeId: "only",
			status: "done",
...
// Scenario 8: hook writes tool.executed evidence line
{
	...
	for (const hook of hooks)
		await hook({ toolName: "bash", eventId: "e-hook", ts: "2026-08-29T14:00:00.000Z", cls: "success", exitCode: 0 }, { cwd: scratch });
	const ev = await readEvents();
	ok(
		"tool.executed evidence line present",
		ev.some((e) => e.event === "tool.executed" && e.eid === "e-hook" && e.tool === "bash"),
	);
}
...
// Scenario 9: missing baseline still passes with note
{
	...
	await attach(
		join(scratch, ".pi/swarm/traces/events.jsonl"),
		`${JSON.stringify({ ts: "2026-08-29T15:00:00.000Z", event: "tool.executed", eid: "e-base", tool: "bash", isError: false, cls: "success", exitCode: 0 })}\n`,
	);
	const res = await as("worker-h", false, () =>
		call(tools, "swarm_update_task", {
			taskId,
			nodeId: "only",
			status: "done",
			...
			attestations: [
				{ claim: "Passed: baseline missing still okay", tool: "bash", eventId: "e-base", ts: "2026-08-29T15:00:00.000Z" },
			],
			cwd: scratch,
		}),
	);
...
// Scenario 10: non-target outcome ignores attestation claims
{
	...
	const ct = await call(tools, "swarm_create_task", {
		...
		priority: "normal",
		cwd: scratch,
		nodes: { only: { role: "planner", terminal: true } },
		edges: [],
	});
	const taskId = ct.content[0].text.match(/task-[A-Za-z0-9-]+/)[0];
	...
	const res = await as("planner-x", false, () =>
		call(tools, "swarm_update_task", {
			...
			attestations: [
				{ claim: "Passed: ignored claim on non-target outcome", tool: "bash", eventId: "e-nope", ts: "2026-08-29T16:00:00.000Z" },
			],
			cwd: scratch,
		}),
	);
