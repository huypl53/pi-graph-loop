...
st.goal = {
	id: "goal-r27",
	text: "R27 lane: task-independent floor",
	setAt: t0,
	setBy: "root",
	origin: "user",
...
writeFileSync(
	...
	JSON.stringify(
		{
			version: 1,
			taskId: "task-r27-open",
			title: "R27 open assigned task",
			goal: "repro",
			status: "in_progress",
			priority: "normal",
			createdAt: t0,
...
ok(
	"no nudge before the streak completes (2/3 samples)",
	sentMessages.length === 0,
	JSON.stringify(sentMessages.map((m) => m.content.slice(0, 60))),
);
await new Promise((r) => setTimeout(r, 1050));
...
ok(
	"surfaced content is the idle-streak goal nudge",
	sentBodies.some((b) => b.includes("Idle streak") || b.includes("goal-r27")),
	JSON.stringify(sentBodies.map((b) => b.slice(0, 80))),
);
const traceLines = existsSync(p.events)
...
ok(
	"streak counter climbed 1→2→3",
	checks
		.slice(0, 3)
		.map((c) => c.count)
		.join(",") === "1,2,3",
	JSON.stringify(checks.map((c) => c.count)),
);
ok(
	"no goal.nudge.error across the whole lane",
	!traceLines.some((e) => e.event === "goal.nudge.error"),
	traceLines
		.filter((e) => e.event === "goal.nudge.error")
		.map((e) => e.error)
		.join(";"),
);
ok(
	"no pre-R27 task-state suppression traces",
	!traceLines.some((e) => e.event === "goal.nudge.suppressed_by_active_task" || e.event === "goal.nudge.deferred_actionable_graph"),
	traceLines
		.filter((e) => String(e.event).startsWith("goal.nudge."))
		.map((e) => e.event)
		.join(";"),
);
const stAfter = await readState(p, dir);
ok(
	"streak reset after emission",
	(stAfter.idleNudgeState?.goalIdleCheckCount ?? 0) <= 1,
	`count=${stAfter.idleNudgeState?.goalIdleCheckCount}`,
);
ok(
	"consecutiveNoResolveNudges climbed to 1",
	stAfter.goal?.consecutiveNoResolveNudges === 1,
	`nudges=${stAfter.goal?.consecutiveNoResolveNudges}`,
);
