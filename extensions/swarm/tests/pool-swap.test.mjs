...
// first two gated strikes. The 3rd strike opens the gate; gated count for this burst is exactly 2.
{
	...
	const gated = raw
		...
		.filter((l) => {
			try {
				return JSON.parse(l).event === "pool.swap_gated_by_engine_retry";
			} catch {
				return false;
			}
		}).length;
...
ok(
	"1st transient does NOT bench (streak 1 < maxRetries 2)",
	!gpt.inCooldown && gpt.health.failures === 1,
	`inCooldown=${gpt.inCooldown} failures=${gpt.health?.failures}`,
);
