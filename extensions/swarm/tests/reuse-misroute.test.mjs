...
console.log("\n[1] plan-reviewer is excluded from a strict `reviewer` reuse lookup (Issue 10 fix)");
{
	...
	ok(
		"only reviewer-01 matched",
		got.length === 1 && got[0].agentId === "reviewer-01",
		`got=${JSON.stringify(got.map((m) => m.agentId))}`,
	);
...
console.log("\n[2] reviewer-01 is excluded from a strict `planner` reuse lookup (negative-id symmetry)");
{
	...
	ok(
		"only planner-reviewer matched",
		got.length === 1 && got[0].agentId === "planner-reviewer",
		`got=${JSON.stringify(got.map((m) => m.agentId))}`,
	);
	ok("matchKind=exact", got[0]?.matchKind === "exact");
}
...
console.log("\n[3] Same-task active-lease guard excludes a busy agent on the SAME task");
{
	...
	const st = {
		agents: {
			"reviewer-busy": mkAgent("reviewer-busy", "Senior reviewer", "reviewer", {
				activeTaskIds: ["T"],
				runtimeStatus: "busy",
	...
	ok(
		"busy same-task agent excluded",
		!got.some((m) => m.agentId === "reviewer-busy"),
		`got=${JSON.stringify(got.map((m) => m.agentId))}`,
	);
	ok(
		"idle agent still included",
		got.some((m) => m.agentId === "reviewer-idle"),
		`got=${JSON.stringify(got.map((m) => m.agentId))}`,
	);
}
...
console.log("\n[4] Same-task active-lease guard keeps an agent when its pointer is stale but it has settled (idle carve-out)");
{
	...
	const st = {
		agents: {
			"reviewer-busy-stale": mkAgent("reviewer-busy-stale", "Senior reviewer", "reviewer", {
				activeTaskIds: ["T"],
				runtimeStatus: "idle",
			}),
		},
	...
	const got = matchReusableAgents(st, { roleKind: "reviewer", excludeTaskId: "T" });
	ok(
		"idle-stale-pointer agent IS included (reclaim path owns the fix)",
		got.length === 1 && got[0].agentId === "reviewer-busy-stale",
		`got=${JSON.stringify(got.map((m) => m.agentId))}`,
	);
}
...
console.log("\n[5] agentId escape-hatch bypasses same-task active-lease guard");
{
	...
	ok(
		"escape-hatch: explicit agentId wins over same-task guard",
		got.length === 1 && got[0].agentId === "reviewer-busy",
		`got=${JSON.stringify(got.map((m) => m.agentId))}`,
	);
...
console.log("\n[6] capabilities escape-hatch matches across role kinds");
{
	...
	ok(
		"without caps: only reviewer-01",
		noCaps.length === 1 && noCaps[0].agentId === "reviewer-01",
		`got=${JSON.stringify(noCaps.map((m) => m.agentId))}`,
	);
	...
	ok(
		"with caps: qa-tester matchKind=fallback",
		withCaps.some((m) => m.agentId === "qa-tester" && m.matchKind === "fallback"),
	);
}
...
console.log("\n[7] substring-collapsed matchKind is recorded when roleKind field would pass but id re-derives to a different kind");
{
	...
	const got = matchReusableAgents(st, { roleKind: "reviewer" });
	ok(
		"substring-collapsed agent still included (field-equality wins)",
		got.length === 1 && got[0].agentId === "planner-reviewer",
		`got=${JSON.stringify(got.map((m) => m.agentId))}`,
	);
