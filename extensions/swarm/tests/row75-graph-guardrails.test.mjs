...
const readMailbox = (agentId) =>
	readFileSync(join(scratch, `.pi/swarm/mailboxes/${agentId}.jsonl`), "utf8")
		.split("\n")
		.filter(Boolean)
		.map((l) => JSON.parse(l));
...
try {
	...
	ok(
		"root mailbox has graph-stall message",
		mb.some((m) => /graph-stall/.test(m.idempotencyKey || "") || /actionable but unassigned/.test(m.subject || "")),
	);
	...
	const closed = await autoCloseRootTerminalNodes(
		fakePi,
		taskPaths({ tasksDir: join(scratch, ".pi/swarm/tasks"), root: scratch }, task3),
		task3State,
	);
	...
	const mixedPi = {
		exec: async (cmd, args) =>
			cmd === "git" && args?.[0] === "rev-parse"
				? { code: 0, stdout: "post-create-head\n", stderr: "" }
	...
	await call("swarm_create_task", {
		...
		cwd: scratch,
		nodes: {
			plan: { role: "planner", dependsOn: [] },
			implement: { role: "implementer", dependsOn: ["plan"] },
			finalize: { role: "root", dependsOn: ["implement"], terminal: true },
		},
		edges: [
			{ from: "plan", to: "implement", when: "planned" },
			{ from: "implement", to: "finalize", when: "implemented" },
		],
		start: "plan",
	});
	...
	const closed4 = await autoClose4(
		fakePiFinalize,
		taskPaths({ tasksDir: join(scratch, ".pi/swarm/tasks"), root: scratch }, task4),
		task4State,
	);
	...
	ok(
		"printGraphText still surfaces finalize evidence via read-compat alias",
		printGraphText(task4State, [], []).includes("Commit evidence [finalize]"),
	);
	...
	const stagedPi = {
		...
		exec: async (cmd, args) => {
			if (cmd === "git" && args?.[0] === "rev-parse") {
				stagedPi.calls += 1;
				return stagedPi.calls === 1
					? { code: 0, stdout: "baseline-before-create\n", stderr: "" }
					: { code: 0, stdout: "post-create-head\n", stderr: "" };
			}
			return { code: 0, stdout: "", stderr: "" };
		},
	...
	await call("swarm_assign_task", {
		...
		nodeId: "plan",
		agentId: "writer-2",
		note: "see ./artifacts/plan.md and artifacts/other.md for context",
		cwd: scratch,
	});
