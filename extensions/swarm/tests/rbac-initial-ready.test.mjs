...
// --- RBAC server-side checks (regular worker identity) ---
{
	...
	ok(
		"worker create_task is rejected (ROOT_AUTHORITY_REQUIRED)",
		thrown && /ROOT_AUTHORITY_REQUIRED/.test(thrown.message || String(thrown)),
	);
...
// --- Initial-ready nudge fires after grace, never auto-assigns ---
{
	...
	ok(
		"reconcile does NOT auto-assign the start node",
		taskJson.nodes[taskJson.start].status !== "assigned" && !taskJson.nodes[taskJson.start].assignee,
	);
	...
	const nudgle = JSON.parse(
		mailbox
			.split(/\n+/)
			.filter(Boolean)
			.reverse()
			.find((l) => l.includes(`nudge:initial-ready`)) || "{}",
	);
	...
	ok(
		"nudge did not auto-assign the start node",
		taskAfter.nodes[taskAfter.start].status !== "assigned" && !taskAfter.nodes[taskAfter.start].assignee,
	);
}
