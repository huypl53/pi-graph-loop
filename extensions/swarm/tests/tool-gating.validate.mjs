...
await runSessionStart();
ok(
	"agent: worker swarm tools present (5)",
	swarmActive().length === WORKER_TOOL_ALLOWLIST.size,
	`${swarmActive().length}/${WORKER_TOOL_ALLOWLIST.size}`,
);
...
await runSessionStart();
ok(
	"root: root swarm tools present (14)",
	swarmActive().length === ROOT_TOOL_ALLOWLIST.size,
	`${swarmActive().length}/${ROOT_TOOL_ALLOWLIST.size}`,
);
...
ok(
	"opt-in end: worker swarm tools re-enabled",
	swarmActive().length === WORKER_TOOL_ALLOWLIST.size,
	`${swarmActive().length}/${WORKER_TOOL_ALLOWLIST.size}`,
);
