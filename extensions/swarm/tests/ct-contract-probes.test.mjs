...
console.log("\n[CT-1.A] pi.sendMessage returns void; no await required");
{
	...
	const ret = pi.sendMessage(
		{ customType: "ct1-probe-A", content: "fire-and-forget", display: true, details: {} },
		{ triggerTurn: true },
	);
	...
	ok(
		"CT-1.A sendMessageReturnIsUndefined === true (no Promise)",
		sendMessageReturnIsUndefined,
		`got ${ret === undefined ? "undefined" : typeof ret}`,
	);
	ok(
		"CT-1.A wrapper invoked synchronously on same tick (no microtask gap)",
		sendMessageWasCalledOnSameTick === true && tick1 - tick0 < 5,
		`deltaMs=${tick1 - tick0}`,
	);
	ok("CT-1.A opts.triggerTurn === true was passed through", sendMessages[0]?.o?.triggerTurn === true, JSON.stringify(sendMessages[0]?.o));

	PROBE_OUTCOME["CT-1.A"] =
		sendMessages.length === 1 && sendMessageReturnIsUndefined && sendMessageWasCalledOnSameTick
			? "CONTRACT_CONFIRMED"
...
console.log("\n[CT-1.B] async error surfaces as runner.emitError, NOT a synchronous throw");
{
	...
	ok("CT-1.B sendMessageCallCount === 1 (wrapper invoked)", sendMessages.length === 1, `got ${sendMessages.length}`);
	ok("CT-1.B emitErrorCallCount === 1 (rejection surfaced async)", emitErrorCalls.length === 1, `got ${emitErrorCalls.length}`);
	ok(
		"CT-1.B emitErrorEvent === 'send_message'",
		Boolean(sendMessageError && sendMessageError.event === "send_message"),
		JSON.stringify(sendMessageError),
	);
	ok(
		"CT-1.B emitErrorErrorContains === 'simulated downstream failure'",
	...
	ok(
		"CT-1.B synchronousThrowOnCaller === false (rejection is async, NOT sync throw)",
		synchronousThrowOnCaller === false,
		`got ${synchronousThrowOnCaller}`,
	);

	PROBE_OUTCOME["CT-1.B"] =
...
console.log("\n[CT-1.C] pre-reload ctx still usable after await ctx.reload(); async error surfaces");
{
	...
	ok("CT-1.C pre-reload baseline sendMessage OK", preReloadSendCount === 1, `got ${preReloadSendCount}`);
	...
	ok(
		"CT-1.C sendMessageCallCount === 2 (pre-reload + post-reload; wrapper IS invoked on pre-reload instance)",
		sendMessages.length === 2,
		`got ${sendMessages.length}`,
	);
	ok(
		"CT-1.C preReloadInstanceStillRuns === true (the captured pre-reload pi is still callable)",
		preReloadInstanceStillRuns,
		`preReloadCounter=${preReloadCounter}`,
	);
	ok(
		"CT-1.C emitErrorCallCount === 1 (post-reload rejection surfaces via runner.emitError)",
		emitErrorCalls.length === 1,
		`got ${emitErrorCalls.length}`,
	);
	ok(
		"CT-1.C emitErrorErrorContains === 'ctx stale' (simulated stale-session rejection)",
		Boolean(sendMessageError && sendMessageError.error?.includes("ctx stale")),
		JSON.stringify(sendMessageError?.error),
	);

	PROBE_OUTCOME["CT-1.C"] =
		sendMessages.length === 2 &&
		preReloadInstanceStillRuns &&
		emitErrorCalls.length === 1 &&
...
console.log("\n[CT-2.A] within 5s of worker send: 0 pi.sendMessage + 1 durable mailbox append");
{
	...
	ok(
		"CT-2.A sendMessageCallCount@T+5s === 0 (root mid-turn; no surface within 5s)",
		sendMessages.length === 0,
		`got ${sendMessages.length}`,
	);
	ok("CT-2.A mailboxAppendCount === 1 (durable contract intact)", mailbox.length === 1, `got ${mailbox.length}`);
	ok(
		"CT-2.A tmuxSendKeysCallCount === 0 (root has unknown target; no pane injection)",
		tmuxSendKeysCalls.length === 0,
		`got ${tmuxSendKeysCalls.length}`,
	);

	PROBE_OUTCOME["CT-2.A"] =
		sendMessages.length === 0 && mailbox.length === 1 && tmuxSendKeysCalls.length === 0
			? "CONTRACT_CONFIRMED"
			: "BUG_FOUND_R_ROW_NEEDED";
}
...
console.log("\n[CT-2.B] after root agent_settled: 1 pi.sendMessage with triggerTurn=true");
{
	...
	ok(
		"CT-2.B sendMessages.length === 0 after busy tick (mid-turn suppression preserved)",
		sendMessagesAfterBusy === 0,
		`got ${sendMessagesAfterBusy}`,
	);
	ok(
		"CT-2.B sendMessageCallCount@postSettled === 1 (agent_settled surfaces the message)",
		postSettledSendMessages === 1,
		`got ${postSettledSendMessages}`,
	);
	ok(
		"CT-2.B opts.triggerTurn === true (the root surfaces as a real trigger)",
		firstPostSettled?.o?.triggerTurn === true,
		JSON.stringify(firstPostSettled?.o),
	);

	PROBE_OUTCOME["CT-2.B"] =
		sendMessagesAfterBusy === 0 && postSettledSendMessages === 1 && firstPostSettled?.o?.triggerTurn === true
			? "CONTRACT_CONFIRMED"
			: "BUG_FOUND_R_ROW_NEEDED";
}
...
console.log("\n[CT-2.C] NO duplicate surface on replay (consumerReceipts dedupe)");
{
	...
	ok("CT-2.C first surface === 1 (initial agent_settled fires once)", firstSurfaceCount === 1, `got ${firstSurfaceCount}`);
	ok(
		"CT-2.C replay sendMessageCallCount === 1 (consumerReceipts dedupes; no duplicate)",
		replaySurfaceCount === 1,
		`got ${replaySurfaceCount}`,
	);
	ok("CT-2.C consumerReceiptsEntryCount >= 1 (dedupe ledger has the entry)", consumerReceiptsCount >= 1, `got ${consumerReceiptsCount}`);

	PROBE_OUTCOME["CT-2.C"] =
		firstSurfaceCount === 1 && replaySurfaceCount === 1 && consumerReceiptsCount >= 1 ? "CONTRACT_CONFIRMED" : "BUG_FOUND_R_ROW_NEEDED";
}
