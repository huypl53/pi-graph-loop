...
// ============================================================
{
	...
	ok("failed test leaves fix pending until assigned", task.nodes.fix.status === "pending");
	const events = await readGlobalEvents();
	ok("failed path leaves no reopen trace", !events.some((e) => e.event === "task.attempt.reopened_by_rework" && e.nodeId === "fix"));
}
