...
import {
	ERR_RECONCILE_RATE_LIMITED,
	ERR_SCOPE_FORBIDDEN,
	PI_SWARM_MINIMAL_PROTOCOL,
	PI_SWARM_RECONCILE_DRYRUN_WORKER_RATE_MS,
...
export function registerMessagesTools(pi: ExtensionAPI) {
	pi.registerTool(
		defineTool({
			...
			label: "Swarm Send",
			description:
				"Send an inter-agent swarm message. The message is appended to the recipient mailbox JSONL and injected into the recipient tmux pane with PI-SWARM system headers when possible.",
			...
			parameters: Type.Object({
				...
				requiresResponse: Type.Optional(
					Type.Boolean({
						description:
							"Whether recipient must send a result/reply message before acking done and becoming reusable. Defaults to false for direct messages.",
					}),
				),
				ttlMs: Type.Optional(
					Type.Number({ description: "Optional time-to-live in milliseconds for future reconcile/dead-letter handling." }),
				),
				idempotencyKey: Type.Optional(
					Type.String({
						description:
							"Optional idempotency key to prevent duplicate messages. If a message with the same from+to+idempotencyKey exists, it is returned instead.",
					}),
				),
			}),
	...
	pi.registerTool(
		defineTool({
			...
			label: "Swarm Ack",
			description:
				"Acknowledge swarm message processing status. Use this after you have seen, completed, or failed a message-triggered task.",
			promptGuidelines: [
				"Use `swarm_ack_message` after processing a swarm message, especially when the message requires acknowledgement.",
			],
			parameters: Type.Object({
				...
				resultMessageId: Type.Optional(
					Type.String({ description: "Optional reply/result message id produced from this message." }),
				),
			...
			async execute(_id, params, _signal, _onUpdate, ctx) {
				return wrapSwarmToolInvocation(pi, ctx.cwd, "swarm_ack_message", async () => {
					...
					const result = await withLock(p, async () => {
						...
						if (rec.to !== agentId && agentId !== "root")
							throw new Error(`Message ${params.messageId} belongs to ${rec.to}, not ${agentId}`);
						const ackAt = now();
						...
						if (rec.superseded) {
							...
							if (progressing && !(done && isOrch && params.waive)) {
								throw new Error(
									`ASSIGNMENT_SUPERSEDED: message ${params.messageId} was superseded by ${rec.superseded.supersededBy}. Complete the current assignment instead. (Root may pass waive=true to accept.)`,
								);
							}
						}
						...
						if (waiveAccept) {
							...
							if (!resultId)
								throw new Error(
									`RESPONSE_REQUIRED: Message ${params.messageId} requires a response before ack done. Send swarm_send_message(to="${rec.from}", replyTo="${rec.id}", ...) first, then ack done with resultMessageId.`,
								);
						...
						st.messages[params.messageId] = {
							...
							lastError: failed
								? params.note || rec.lastError
								: rec.lastError?.startsWith("ack_missing")
									? undefined
									: rec.lastError,
							response,
							lastAck: {
	...
	pi.registerTool(
		defineTool({
			...
			promptGuidelines: [
				"Use `swarm_message_status` to debug whether swarm messages are queued, mailbox_delivered, injected, intercepted, acked, failed, or dead_letter.",
			],
			parameters: Type.Object({
			...
			async execute(_id, params, _signal, _onUpdate, ctx) {
				return wrapSwarmToolInvocation(pi, ctx.cwd, "swarm_message_status", async () => {
					...
					records = records
						...
						.slice(-Math.max(1, Math.min(200, params.limit || 50)));
			...
			async execute(_id, params, _signal, _onUpdate, ctx) {
				return wrapSwarmToolInvocation(pi, ctx.cwd, "swarm_check_mailbox", async () => {
					...
					const result = await withLock(p, async () => {
						...
						await trace(p, "mailbox.poll", {
							agentId,
							count: messages.length,
							matchedCount,
							pendingOnly: Boolean(params.pendingOnly),
							markDelivered: Boolean(params.markDelivered),
						});
		...
		defineTool({
			...
			parameters: Type.Object({
				agentId: Type.Optional(
					Type.String({
						description:
							"Optional agent id to reconcile only that agent's messages. Task sweep is skipped when scoped to one agent.",
					}),
				),
				dryRun: Type.Optional(
					Type.Boolean({ description: "If true, inspect and report actions without modifying state. Defaults to false." }),
				),
				mark: Type.Optional(
					Type.Boolean({
						description:
							"Persist the recomputed task.status when stored/derived drift is detected (repairs closure). Still never auto-fails nodes. Defaults to false.",
					}),
				),
				scope: Type.Optional(
					Type.Union([Type.Literal("self"), Type.Literal("all")], {
						description:
							"Reconcile scope: 'self' restricts to the caller's mailbox; 'all' walks the whole swarm. Workers are forced to 'self'; root/admin may select either. Defaults to caller-tier (worker=self, root=all).",
					}),
				),
				offset: Type.Optional(
					Type.Number({
						minimum: 0,
						description:
							"Zero-based action offset. Skips this many actions from the current deterministic reconcile result; pass the prior result's nextOffset to avoid returning old actions again. Defaults to 0.",
					}),
				),
			}),
			async execute(_id, params, _signal, _onUpdate, ctx) {
				return wrapSwarmToolInvocation(pi, ctx.cwd, "swarm_reconcile", async () => {
					...
					if (!isPrivileged && requestedScope !== "self") {
						throw new Error(
							`${ERR_SCOPE_FORBIDDEN}: worker ${me} requested scope="${requestedScope}" but only "self" is permitted (proposal §K.1).`,
						);
					}
						...
						await withLock(p, async () => {
							...
							if (a) {
								...
								if (lastAt > 0 && nowMs - lastAt < PI_SWARM_RECONCILE_DRYRUN_WORKER_RATE_MS) {
									...
									throw new Error(
										`${ERR_RECONCILE_RATE_LIMITED}: worker ${me} dry-run reconcile throttled; retry in ${waitS}s (proposal §K.1).`,
									);
								}
					...
					const result = await reconcile(pi, ctx.cwd, p, {
					...
					const page =
						result.offset > 0 || result.count !== result.totalCount
					...
					return textResult(
						`${page} (${result.dryRun ? "dry run" : "applied"}). Next offset: ${result.nextOffset}.\n${summary}`,
						result,
					);
				});
			},
