...
function rowKey(v: string): string {
...
function safeHint(command: string): string {
	return command;
}
...
async function resolveTaskRefLocal(
	p: Paths,
	ref: string,
...
export class FlowDialog implements Component {
	...
	private selectedRow(): Row | undefined {
		const rows = this.rows();
		return rows[Math.min(this.selected, Math.max(0, rows.length - 1))];
	}
	handleInput(data: string): void {
		if (!this.debugRaw) {
	...
	private handleLegacyInput(data: string): void {
		...
		if (matchesKey(data, Key.left) || data === "h") {
			if (this.hOffset > 0) {
				this.hOffset = Math.max(0, this.hOffset - 10);
				this.tui.requestRender();
			}
			return;
		}
		...
		if (matchesKey(data, Key.down) || data === "j") {
			...
			if (rows.length) this.selected = (this.selected + 1) % rows.length;
			this.tui.requestRender();
			return;
		}
		if (matchesKey(data, Key.enter)) {
	...
	private selectedGraphEntry(): GraphTreeEntry | undefined {
	...
	private handleInputV3(data: string): void {
		...
		if (this.help) {
			return;
		} // Help mode only responds to Esc
		if (this.filterMode) {
			if (matchesKey(data, Key.backspace)) {
				this.filter = this.filter.slice(0, -1);
				this.tui.requestRender();
		...
		if (this.fullBodyMessageId !== null) {
			...
			if (data === "j") {
				this.messageViewScroll += 1;
				this.tui.requestRender();
				return;
			}
			if (data === "k") {
				this.messageViewScroll = Math.max(0, this.messageViewScroll - 1);
				this.tui.requestRender();
				return;
			}
			if (data === "r") {
	...
	private renderHeader(W: number): string[] {
		...
		if (!snap)
			return [
				this.fg("border", "╭" + "─".repeat(Math.max(0, W - 2)) + "╮"),
				this.fg("border", "│ ") + this.pad(this.fg("muted", "loading…"), innerW) + this.fg("border", " │"),
				this.fg("border", "╰" + "─".repeat(Math.max(0, W - 2)) + "╯"),
			];
		...
		const out: string[] = [];
		out.push(
			this.fg("border", "╭─") + this.fg("accent", ttl) + this.fg("border", "─".repeat(Math.max(1, W - 3 - visibleWidth(ttl))) + "╮"),
		);
		out.push(
			this.fg("border", "│ ") +
				this.pad(this.fg("dim", `open=${snap.open} stale=${snap.staleCount}`), innerW) +
				this.fg("border", " │"),
		);
		if (this.filterMode || this.filter)
			out.push(
				this.fg("border", "│ ") +
					this.pad(this.fg("accent", `/ ${this.filter}${this.filterMode ? "▏" : ""}`), innerW) +
					this.fg("border", " │"),
			);
		return out;
	}
	render(width: number): string[] {
		return this.debugRaw ? this.renderLegacy(width) : this.renderV3(width);
	}
	private renderV2(width: number): string[] {
		...
		out.push(
			this.fg("border", "│ ") +
				this.pad(this.fg("dim", `refreshed ${clock(snap.refreshedAt)} · ${snap.freshnessLabel}`), innerW) +
				this.fg("border", " │"),
		);
		...
		if (!graphLines.length)
			out.push(this.fg("border", "│ ") + this.pad(this.fg("muted", "  (no nodes)"), innerW) + this.fg("border", " │"));
		for (const line of graphLines) {
			const full = `  ${line}`;
			this.hMax = Math.max(this.hMax, visibleWidth(full));
			out.push(this.fg("border", "│ ") + this.hScroll(full, innerW) + this.fg("border", " │"));
		}
		for (const line of buildHandoffLines(snap.task, snap.st).slice(0, 4))
			out.push(this.fg("border", "│ ") + this.hScroll(this.fg("dim", `  ${line.text}`), innerW) + this.fg("border", " │"));
		if (currentIds.length) {
			...
			out.push(
				this.fg("border", "│ ") +
					this.pad(this.fg("dim", `current: ${currentIds.length > 1 ? "parallel current · " : ""}${current}`), innerW) +
					this.fg("border", " │"),
			);
		}
		const rows = this.rows();
		...
		// Sliding window + selected highlight for attention rows (V2).
		{
			const rowsAll = rows;
			const limit = 4;
			const selIdxInSec = attRows.findIndex(
				(r) => rowsAll[this.selected] && r.id === rowsAll[this.selected].id && r.section === rowsAll[this.selected].section,
			);
			let start = 0;
			...
			if (start > 0)
				out.push(
					this.fg("border", "│ ") +
						this.pad(this.fg("muted", `  ↑ +${start} earlier in attention`), innerW) +
						this.fg("border", " │"),
				);
			for (const row of attRows.slice(start, start + limit)) {
				const selected =
					rowsAll[this.selected] && rowsAll[this.selected].id === row.id && rowsAll[this.selected].section === row.section;
				const body = this.fg(row.severity === "act" ? "error" : row.severity === "watch" ? "warning" : "muted", `  ${row.summary}`);
				out.push(
					this.fg("border", "│ ") +
						(selected ? this.bg("selectedBg", this.pad(body, innerW)) : this.pad(body, innerW)) +
						this.fg("border", " │"),
				);
			}
			if (start + limit < attRows.length)
				out.push(
					this.fg("border", "│ ") +
						this.pad(this.fg("muted", `  ↓ +${attRows.length - start - limit} more in attention`), innerW) +
						this.fg("border", " │"),
				);
		}
		for (const section of ["FLOW", "LANES"] as Section[]) {
			...
			if (!secRows.length) {
				out.push(this.fg("border", "│ ") + this.pad(this.fg("muted", "  (none)"), innerW) + this.fg("border", " │"));
				continue;
			}
			...
			const selIdxInSec = secRows.findIndex(
				(r) => rows[this.selected] && r.id === rows[this.selected].id && r.section === rows[this.selected].section,
			);
			let start = 0;
			if (selIdxInSec >= limit) start = selIdxInSec - limit + 1;
			if (start > 0)
				out.push(
					this.fg("border", "│ ") +
						this.pad(this.fg("muted", `  ↑ +${start} earlier in ${section.toLowerCase()}`), innerW) +
						this.fg("border", " │"),
				);
			for (const row of secRows.slice(start, start + limit)) {
				...
				out.push(
					this.fg("border", "│ ") +
						(selected ? this.bg("selectedBg", this.pad(`  ${label}`, innerW)) : this.pad(`  ${label}`, innerW)) +
						this.fg("border", " │"),
				);
			}
			if (start + limit < secRows.length)
				out.push(
					this.fg("border", "│ ") +
						this.pad(this.fg("muted", `  ↓ +${secRows.length - start - limit} more in ${section.toLowerCase()}`), innerW) +
						this.fg("border", " │"),
				);
			if (section === "LANES" && !this.expandLanes && snap.otherAgentsCount > 0)
				out.push(
					this.fg("border", "│ ") +
						this.pad(this.fg("muted", `  + ${snap.otherAgentsCount} other agents collapsed (press o to expand)`), innerW) +
						this.fg("border", " │"),
				);
		}
		{
			...
			const limit = 6;
			const selIdxInSec = evRows.findIndex(
				(r) => rows[this.selected] && r.id === rows[this.selected].id && r.section === rows[this.selected].section,
			);
			let start = 0;
			if (selIdxInSec >= limit) start = selIdxInSec - limit + 1;
			if (start > 0)
				out.push(
					this.fg("border", "│ ") +
						this.pad(this.fg("muted", `  ↑ +${start} earlier in events`), innerW) +
						this.fg("border", " │"),
				);
			...
			for (const row of evRows.slice(start, start + limit)) {
				...
				const prefix =
					groupTitle && groupTitleById.get(evRows[start]?.id || "") !== groupTitle
						? this.fg("dim", `  ${groupTitle}: `)
						: this.fg("dim", "  ");
			...
			if (start + limit < evRows.length)
				out.push(
					this.fg("border", "│ ") +
						this.pad(this.fg("muted", `  ↓ +${evRows.length - start - limit} more in events`), innerW) +
						this.fg("border", " │"),
				);
		}
		if (this.nextText)
			out.push(this.fg("border", "│ ") + this.pad(this.fg("dim", `NEXT: ${this.nextText}`), innerW) + this.fg("border", " │"));
		...
		out.push(
			this.fg("border", "│ ") +
				this.pad(
					this.fg(
						"dim",
						`${hInfo}↑↓jk nav · ←→hl scroll · Tab section · Enter detail · / filter · o lanes · d raw · r refresh · ? help · Esc`,
					),
					innerW,
				) +
				this.fg("border", " │"),
	...
	private renderV3(width: number): string[] {
		...
		if (this.fullBodyMessageId !== null && this.messageViewNodeId !== null) {
			...
			if (msg) {
				...
				out.push(
					this.fg("border", "╭─") +
						this.fg("accent", header) +
						this.fg("border", "─".repeat(Math.max(1, W - 3 - visibleWidth(header))) + "╮"),
				);
				const subject = msg.subject || "(no subject)";
				out.push(this.fg("border", "│ ") + this.pad(this.fg("accent", subject), innerW) + this.fg("border", " │"));
				out.push(
					this.fg("border", "│ ") +
						this.pad(this.fg("dim", "─".repeat(Math.max(0, innerW - 2))), innerW) +
						this.fg("border", " │"),
				);
				...
				if (bodyStart > 0)
					out.push(
						this.fg("border", "│ ") + this.pad(this.fg("muted", `  ↑ +${bodyStart} earlier`), innerW) + this.fg("border", " │"),
					);
				...
				if (bodyStart + limit < bodyLines.length)
					out.push(
						this.fg("border", "│ ") +
							this.pad(this.fg("muted", `  ↓ +${bodyLines.length - bodyStart - limit} more`), innerW) +
							this.fg("border", " │"),
					);
				out.push(
					this.fg("border", "│ ") +
						this.pad(this.fg("dim", "j/k scroll · Esc back to message list"), innerW) +
						this.fg("border", " │"),
				);
				out.push(this.fg("border", "╰" + "─".repeat(Math.max(0, W - 2)) + "╯"));
				return out;
		...
		if (this.messageViewNodeId !== null) {
			...
			if (nodeMessages.length === 0) {
				out.push(
					this.fg("border", "│ ") + this.pad(this.fg("muted", "no messages for this node"), innerW) + this.fg("border", " │"),
				);
			} else {
				const limit = 8;
				...
				if (start > 0)
					out.push(
						this.fg("border", "│ ") + this.pad(this.fg("muted", `  ↑ +${start} earlier`), innerW) + this.fg("border", " │"),
					);
				...
				if (start + limit < nodeMessages.length) {
					out.push(
						this.fg("border", "│ ") +
							this.pad(this.fg("muted", `  ↓ +${nodeMessages.length - start - limit} more`), innerW) +
							this.fg("border", " │"),
					);
				}
			}
			out.push(
				this.fg("border", "│ ") +
					this.pad(this.fg("dim", "j/k scroll · Enter msg body · Esc back to graph"), innerW) +
					this.fg("border", " │"),
			);
			out.push(this.fg("border", "╰" + "─".repeat(Math.max(0, W - 2)) + "╯"));
			return out;
		...
		if (this.fullBodyMessageId !== null) {
			...
			if (msg) {
				...
				if (bodyStart + 10 < bodyLines.length) {
					out.push(
						this.fg("border", "│ ") +
							this.pad(this.fg("muted", `  ↓ +${bodyLines.length - bodyStart - 10} more`), innerW) +
							this.fg("border", " │"),
					);
				}
			} else {
		...
		for (const line of detailLines) {
			for (const ln of wrapTextWithAnsi(line, Math.max(20, innerW)))
				out.push(this.fg("border", "│ ") + this.pad(ln, innerW) + this.fg("border", " │"));
		}
		out.push(
			this.fg("border", "│ ") +
				this.pad(this.fg("dim", "j/k navigate · Enter messages · ? help · d raw · r refresh · Esc close"), innerW) +
				this.fg("border", " │"),
		);
		out.push(this.fg("border", "╰" + "─".repeat(Math.max(0, W - 2)) + "╯"));
		return out;
		...
		for (const section of ["ATTENTION", "FLOW", "LANES", "EVENTS"] as Section[]) {
			...
			if (section === "FLOW")
				out.push(
					this.fg("border", "│ ") +
						this.pad(
							this.fg("dim", `Ready: ${snap.ready.join(", ") || "(none)"} · Current: ${snap.current.join(", ") || "(none)"}`),
							innerW,
						) +
						this.fg("border", " │"),
				);
	...
	private renderHelp(W: number): string[] {
		...
		out.push(
			this.fg("border", "╭─") +
				this.fg("accent", " swarm flow help ") +
				this.fg("border", "─".repeat(Math.max(1, W - 3 - visibleWidth(" swarm flow help "))) + "╮"),
		);
		for (const [k, v] of [
			["navigate", "↑↓ or j / k"],
			...
			out.push(
				this.fg("border", "│ ") + this.pad(`${this.fg("accent", k)} · ${this.fg("dim", v)}`, innerW) + this.fg("border", " │"),
			);
		out.push(this.fg("border", "╰" + "─".repeat(Math.max(0, W - 2)) + "╯"));
		return out;
	}
	private renderDetail(W: number, item: Row): string[] {
		...
		for (const ln of wrapTextWithAnsi(item.detail, Math.max(20, innerW)))
			out.push(this.fg("border", "│ ") + this.pad(truncateToWidth(ln, innerW), innerW) + this.fg("border", " │"));
		out.push(this.fg("border", "│ ") + this.pad(this.fg("dim", `Hint: ${item.hint}`), innerW) + this.fg("border", " │"));
		out.push(
			this.fg("border", "│ ") +
				this.pad(this.fg("dim", `Task: /swarm task ${this.data?.task.taskId || "?"} runtime`), innerW) +
				this.fg("border", " │"),
		);
