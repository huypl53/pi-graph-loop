...
assert.equal(registered.config.models.length, 66, "expected sixty-six scenario models (52 pre-existing + 14 added since the selftest was last calibrated + 3 ct-phase2 fixtures = 66)");
assert.deepEqual((await discoverModelConfigs()).map((model) => model.id).sort(), ["429-mid-edit", "ack-lifecycle-booking", "agent-retirement-lease", "agent-retirement-sweep", "assignment-fence-stale-attempt", "auto-close-evidence-closure", "cancel-supersession", "commit-no-evidence", "dead-letter-final", "delivery-repair-retry", "drift-then-wake", "edit-not-persisted", "gc-retention-dryrun", "goal-busy-epoch-reset", "goal-clear-refusal", "goal-interval-inherit", "goal-interval-reanchor", "graph-advance-nudge-rearm", "handoff-chain", "heartbeat-gc-dead-pane", "idle-nudge-recovery", "initial-ready-nudge", "inprogress-death", "liveness-stale-open", "mailbox-delivery-read", "midturn-assign", "parallel-fail-a", "parallel-fail-b", "pool-engine-retry-gated-swap-exhausted", "pool-quota-bench-cooldown-recovery", "pool-spawn-fallback-all-tagged-benched", "pool-strict-roles-tagged-only", "prune-retention-apply", "priority-high-interrupt", "priority-high-interrupt-rate-limited", "proxy-metrics-emit", "quota-429-then-recovery", "quota-exhausted-all-turns", "reconcile-repair-retry", "response-credit-verified-result", "response-missing-settle", "response-required-death", "settled-with-open-assignment", "shared-context-a", "shared-context-b", "stale-all-agents", "supersession-late-result", "supersession-race-new", "supersession-race-old", "task-graph-semantics", "torn-json-then-recovery", "wake-up-escalation-reminder"].sort());
assert.equal((await listFixtureDiscovery()).length, 66);
