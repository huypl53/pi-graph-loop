...
assert.equal(registered.config.models.length, 69, "expected 69 scenario models");
assert.deepEqual((await discoverModelConfigs()).map((model) => model.id).sort(), ["429-mid-edit","ack-lifecycle-booking","agent-retirement-lease","agent-retirement-sweep","assignment-fence-stale-attempt","audit-invariant-violation","auto-close-evidence-closure","cancel-supersession","commit-no-evidence","ct1-prereload-sendmessage","ct2-r15-surface-bound","ct3-nextturn-idle","ct4-compaction-retry","ct5-session-start-signal","ct6-reload-stale-ctx","ct7-end-vs-settled","dead-letter-final","delivery-repair-retry","drift-then-wake","edit-not-persisted","gc-retention-dryrun","goal-busy-epoch-reset","goal-clear-refusal","goal-interval-inherit","goal-interval-reanchor","goal-nudge-surface-starve","graph-advance-nudge-rearm","handoff-chain","heartbeat-gc-dead-pane","idle-nudge-recovery","initial-ready-nudge","inprogress-death","liveness-stale-open","mailbox-delivery-read","midturn-assign","parallel-fail-a","parallel-fail-b","pool-engine-retry-gated-swap-exhausted","pool-quota-bench-cooldown-recovery","pool-spawn-fallback-all-tagged-benched","pool-strict-roles-tagged-only","priority-high-interrupt","priority-high-interrupt-rate-limited","proxy-metrics-emit","prune-retention-apply","quota-429-then-recovery","quota-exhausted-all-turns","r12-shared-pool-mass-sweep","r13-orchestrator-unknown-target","r14-goal-empty-pool-vacuous","r15-normal-unknown-target-busy","r16-idle-goal-regression","r17-ct2-real-lane","r17-ct2-real-lane-worker","r18-scratch-pi-startup","reconcile-repair-retry","response-credit-verified-result","response-missing-settle","response-required-death","settled-with-open-assignment","shared-context-a","shared-context-b","stale-all-agents","supersession-late-result","supersession-race-new","supersession-race-old","task-graph-semantics","torn-json-then-recovery","wake-up-escalation-reminder"].sort());
assert.equal((await listFixtureDiscovery()).length, 69);
