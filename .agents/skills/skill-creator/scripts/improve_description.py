...
by calling `pi -p` as a subprocess, using the session's pi auth (no separate
key needed). Provider/model default to $PI_PROVIDER / $PI_MODEL.
"""
...
def _call_pi(prompt: str, model: str | None, provider: str | None = None, timeout: int = 300) -> str:
    """Run `pi -p` with the prompt as a positional arg and return the text.

    `--no-skills --no-extensions` keeps this a bare text-generation call (the
    improvement model has no business loading skills or running extensions).
    """
    cmd = ["pi", "--no-extensions", "--no-skills", "-p"]
...
The description appears in the agent's available-skills list. When a user sends a query, the agent decides whether to read (and thus invoke) the skill based solely on the title and on this description. Your goal is to write a description that triggers for relevant queries, and doesn't trigger for irrelevant ones.
...
def main():
    ...
    parser.add_argument("--model", default=None, help="pi model (default: $PI_MODEL)")
